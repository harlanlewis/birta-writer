#!/bin/bash
# SessionEnd hook: clear the development litter a Mac app run leaves on the machine.
#
...
[ -n "$ROOT" ] || exit 0
[ -x "$ROOT/mac/scripts/reap.sh" ] || exit 0

# macOS only. The app does not exist anywhere else, so neither does its litter.
[ "$(uname -s)" = "Darwin" ] || exit 0

output="$(bash "$ROOT/mac/scripts/reap.sh" --reap 2>&1 || true)"
case "$output" in
    *"cleared 0,"*) ;;
    *) echo "$output" ;;
esac
