...
- The harness lock refuses with exit 2. To queue behind a lane's gates, loop on that code and only it (any other exit is the real result); a script wrapping `gate.sh` in that loop beats hand-polling. The trap is worst in a probe you wrote yourself, where a nonzero exit is the signal you are hunting: a mutation runner reported every mutation killed while the lock refused and nothing ran, and it hid a real survivor. Demand the evidence the verdict implies, a counted failure, never the exit code alone.
