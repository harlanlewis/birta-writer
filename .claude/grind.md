...
- The harness lock refuses with exit 2. To queue behind a lane's gates, loop on that code and only it (any other exit is the real result); a script wrapping `gate.sh` in that loop beats hand-polling.
