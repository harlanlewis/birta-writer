...
- The harness lock refuses with exit 2. Loop on that code and only it; any other exit is the real result. The trap is worst in a probe you wrote yourself, where a nonzero exit is the signal you are hunting: a mutation runner called every mutation killed while the lock refused and nothing ran, hiding a real survivor. Demand the evidence a verdict implies, a counted failure, never the exit code alone.
- `cd` persists between Bash calls, so inspecting a lane's worktree silently moves later commands. The worktree hook does NOT cover this: it refuses git redirected elsewhere (`-C`, a path argument), and a bare `cd` passes it, after which plain commands run in the new directory. Restored after being deleted on the claim that the hook had superseded it, which the same session then disproved by drifting.
