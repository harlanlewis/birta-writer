...
Nothing goes into a shared session narrative, and this file is not one: a document only sessions append to has no bound, because no session deletes another's true sentence. A war story is not a finding. The finding is the sentence that changes what the next session does; anything else belongs to the PR body and `git log`.
...
- Merging deploys: no.
- `pnpm run install:local` is the LAST act, after the pull, and then tell the user to reload. Run it before the merge and the user reloads into a build the session's own later commits contradict. Install from the merged default branch or the install is a lie about what shipped. Run `pnpm install` first. Sessions work in worktrees, so the primary checkout's `node_modules` is as old as the last person who installed there, and `install:local` folds in `pnpm test`: Vitest 3 against a tree pinned to 4 fails the session's last act for a reason that is not the session's. The version banner is the tell.
...
- With peer sessions live the harness lock refuses more often than it is free, and `merge-lane.sh --gate` runs its gate once, so merge bare and gate in a separate call that retries on the lock's refusal message (`harness lock: held by`, not exit 2, which is ambiguous). The lock is `$TMPDIR/hl-harness.lock`, one file shared with every repository on the machine (`birta-labs:HARNESS_LOCK.md`), so the holder a refusal names may be a peer session in another repo; its `cwd` says which, and `ListAgents` names who to ask. `pnpm test` holds it as a sweep and runs beside another repo's sweep; the e2e sweep and every `perf:*` capture hold it as a capture and refuse everything. A `gate_failed` on a lock refusal or a timed-out test is a contention red; re-run the failing file alone before blaming the lane. The lock does not see `mac/scripts/test.sh`, `pnpm build` or another repo's unit tests, so a backgrounded `pnpm test` overlapping a Swift run reds `corpusMoveSampling` and `roundTripCorpus` at their per-test timeout; both pass alone. Never write, remove or overwrite that lock file by hand: a peer's live capture may hold it, and a holder heartbeats its record, so a file that reads as held by a live pid is held.
...
- A squash merge leaves every lane branch a non-ancestor of `main`, so `cleanup-worktrees.sh` refuses to delete them and is right to. Ancestry cannot answer whether they landed: diff trees instead, over the files each branch touched. Empty means landed.
- `cd` persists between Bash calls, so inspecting a lane's worktree silently moves later commands, and the worktree hook does not catch it: it refuses git aimed elsewhere (`-C`, a path argument), a heredoc, a `bash <script>` it cannot read, a `for` loop or a `;` chain whose text a shell variable decides, but a bare `cd` passes. In zsh an `echo` word starting with `=` is a command lookup and errors; use `printf`. Write files with the Write tool, and run a multi-step gate as a `node` script by absolute path that retries on the lock's refusal message: a chain that sleeps and retries inside one process is the only way a gate survives a peer lane holding the lock, and a tool call dies at ten minutes either way.
- A lane that has launched its own gate in the background will stop its turn to wait for it and report "waiting" as if finished. Brief it to poll the output file inside one tool call; a lane that does stop has to be resumed by message, and each resume costs a round trip.
