...
- With peer sessions live the harness lock refuses more often than it is free, and `merge-lane.sh --gate` runs its gate once, so merge bare and gate in a separate call that retries on the lock's refusal message (not on exit 2, which is ambiguous). A `gate_failed` on a lock refusal or a timed-out test is a contention red; re-run the failing file alone before blaming the lane. The lock does not see `mac/scripts/test.sh`, so a backgrounded `pnpm test` overlapping a Swift run reds `corpusMoveSampling` and `roundTripCorpus` at their per-test timeout; both pass alone.
