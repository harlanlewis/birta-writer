...
- `cd` persists between Bash calls, so inspecting a lane's worktree silently moves later commands, and the worktree hook does not catch it: it refuses git aimed elsewhere (`-C`, a path argument), a heredoc, a `bash <script>` it cannot read, and any command whose text a shell variable decides, but a bare `cd` passes. Write files with the Write tool, and run a multi-step gate as a `node` script by absolute path that retries on the lock's refusal message: a chain that sleeps and retries inside one process is the only way a gate survives a peer lane holding the lock, and a tool call dies at ten minutes either way.
