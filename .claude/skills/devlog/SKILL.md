...
- Autonomous sessions (e.g. `/grind`) must clear the filing bar before creating an issue (grind §3.6): a nameable user-observable symptom or demonstrated hazard, and a genuine reason it can't be done in-session (blocked on a user decision/design/upstream, or untouched territory). Fix-now beats filing; low-value findings are declined in the session report, not filed. A small-but-real item that genuinely can't be done now is an issue at `priority: Low`, never a line appended to a shared document: a checkbox on a board guide is a ticket with its tracking removed, and it goes unread for as long as it is carried. Never an omnibus "cleanups/follow-ups" ticket: one issue = one nameable outcome. Filing into `Backlog` is cheap for the filer and expensive for the queue.
