...
将两类条目提交为 GitHub Issue（仓库：`harlanlewis/md-wysiwyg-editor`）：
...
```bash
gh issue create \
  --repo harlanlewis/md-wysiwyg-editor \
  --title "[Bug] <标题>" \
...
```bash
gh label list --repo harlanlewis/md-wysiwyg-editor | grep known-limitation
```
...
```bash
gh label list --repo harlanlewis/md-wysiwyg-editor | grep roadmap
```
...
```
✅ Issue 已创建：https://github.com/harlanlewis/md-wysiwyg-editor/issues/XXX
```
