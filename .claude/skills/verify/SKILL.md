...
- If a suite already covers the surface you changed (e.g. `e2e/imageView/`), extend it with new checks instead of building a scratchpad harness, so the checks then guard the change forever.
...
- Heading NodeViews add chrome text ("#H2" fold labels), so never assert block `textContent` equality; assert via `window.__posted` markdown or `includes()`.
...
- Toolbar/TOC buttons stopPropagation on mousedown, so clicks there only reach document-level listeners registered in the capture phase.
