...
/// puts on screen is either a `AppPanel`, which is `.fullScreenAuxiliary` so
...
///
/// To see it: `bash mac/scripts/menu-bar.sh`, which reads the running app's
/// real menu bar through the accessibility API. Nothing in `mac/Tests` can,
