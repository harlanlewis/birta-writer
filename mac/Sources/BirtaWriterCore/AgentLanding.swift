...
/// which is what keeps the answer independent of the autosave setting. The app's
...
/// The Mac app autosaves by default, within half a second of the last
/// keystroke, so the buffer goes over the agent's file almost at once and its
/// output is gone with nothing having said so.
///
/// So the app keeps a copy beside the note, which is the answer it already gives
