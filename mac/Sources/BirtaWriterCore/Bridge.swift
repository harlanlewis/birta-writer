...
/// outbound); the app speaks the subset a scratchpad needs and files everything
...
/// One editor command as the page offers it to the app's palette: the id
/// `editorCommand` runs it by, the title a row reads, and the section it prints
/// under (the host's menu name where the host binds it, else "Editor").
public struct PaletteCommand: Equatable, Sendable {
    public let id: String
    public let title: String
    public let section: String

    public init(id: String, title: String, section: String) {
        self.id = id
        self.title = title
...
public enum WebviewMessage: Equatable {
    ...
    /// `conflict` or `unchanged`. The app acts on it because `partial` and
    /// `conflict` leave the agent's version only in the file, and the app's
    ...
    ///
    /// The app MUST answer, and answering is not optional politeness: the composer
    ...
    /// Embed-card caption for a recognized provider. The app answers it with
    ...
    public static func parse(_ text: String) -> WebviewMessage? {
        ...
        case "paletteCommands":
            ...
            let items = (dict["items"] as? [Any] ?? []).compactMap { entry -> PaletteCommand? in
                guard let row = entry as? [String: Any],
...
public enum HostMessage: Equatable {
    ...
    /// Measurement-only: an arbitrary message object, so `mac/scripts/measure.sh`
...
public struct BootConfig: Equatable {
    ...
    /// the rendered document is the point, and the gutter is for matching it
    /// against a diff or a review comment.
    ...
    public init(toolbarJSON: String = #"{"placements":{},"order":[]}"#,
                ...
                formattingRowExpanded: Bool = false,
        ...
        self.formattingRowExpanded = formattingRowExpanded
    ...
    /// default, so this lists only what the app decides.
    public func i18nObject() -> [String: Any] {
        ...
        return [
            ...
            "isMac": true,
            "toolbar": toolbar,
            // Everything the app says about ITSELF, in the one key the editor reads
            ...
            // told it has a root (`HostMessage.projectRoot`). A boolean under
            // the page's own key (`webview/i18n/index.ts`); the string is this
            // side's storage, kept in the outline panel's spelling.
            "fileExplorerVisible": fileExplorerVisibility == "shown",
            // The formatting row as the reader last left it, in ANY window:
            // the page reads this once as it builds the row, and every later
            // flip anywhere arrives as `setFormattingRowExpanded`.
            "formattingRowExpanded": formattingRowExpanded,
            // The gutter, under the extension's own key: the page decides at
            // boot whether to fetch the gutter's module from this, and a flip
            // later arrives as `setLineNumbers`.
...
/// The toolbar layout the app persists on the host side, in place of the
