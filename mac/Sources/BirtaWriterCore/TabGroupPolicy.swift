...
public enum TabGroupPolicy {
    ...
    /// toolbar row on and the tab bar's row below it, which the page has to
    /// leave clear. Zero tab bar means the whole band is the title row.
    public static func bandSplit(band: Double, tabBar: Double) -> (titleRow: Double, tabBar: Double) {
        let bar = min(max(tabBar, 0), max(band, 0))
        return (max(band, 0) - bar, bar)
    }
