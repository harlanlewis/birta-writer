...
/// side; this is the ONE implementation on the Mac side). The app has one
/// document and one writer, so the per-document and per-writer maps collapse
/// to two integers, and the backend is the single-writer one: only the current
