...
/// WHERE the app keeps the one file a person who has changed no settings ever
...
public enum ScratchpadLocation: String, CaseIterable, Sendable {
    ...
    /// filesystem.
    ///
    /// `MAC_APP_NAME` in shared/product.ts is the source; Swift cannot
    ...
    /// developer account, and the app is ad-hoc signed (mac/scripts/build-app.sh).
