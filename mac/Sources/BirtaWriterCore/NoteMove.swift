...
/// What moving the app's notes from one folder to another would DO, decided
...
///
/// `Prefs.notesDirectory` is the scratchpad's PARENT, so this is a folder the
/// app does not exclusively own. Three rules follow, and the second is the
/// one that is easy to get wrong:
///
///   - Every `.md` at the top level travels, and nothing else does. A file the
///     app did not put there is left where its owner put it, and named in
...
public struct NoteMove: Equatable, Sendable {
    ...
    /// Files in the source folder that are NOT the app's and stay where they are.
    /// The caller names these rather than moving them; a folder the app shares is
    /// not a folder it may empty.
