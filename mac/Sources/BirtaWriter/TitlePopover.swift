...
/// behind it gets the Cmd-click path menu and nothing else. The app is one panel
...
///           stem (`BirtaWriterCore.DocumentName` decides, and is tested)
///   Tags    the Finder tags, as Finder holds them (`BirtaWriterCore.FinderTags`)
...
final class TitlePopoverController: NSViewController {
    ...
    /// What the form is showing, for `mac/scripts/measure.sh`. Every row, so a
