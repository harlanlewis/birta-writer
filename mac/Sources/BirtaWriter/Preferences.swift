...
/// UserDefaults-backed preferences. Everything the app remembers between launches
...
///
/// The app's configuration is wholly its own on purpose: sharing `birta.*`
/// settings with the extension would couple the two release clocks (MAR-370).
enum Prefs {
    /// `BIRTA_MAC_DEFAULTS_SUITE` names a separate defaults domain, so
    /// mac/scripts/measure.sh never rewrites the user's own layout and frame.
    private static let d: UserDefaults = {
        if let suite = ProcessInfo.processInfo.environment["BIRTA_MAC_DEFAULTS_SUITE"], !suite.isEmpty,
    ...
    ///
    /// Everything the app remembers goes through `d` and is therefore covered by
    ...
    /// person's own window at whatever width the run finished on. `AppPanel`
    ...
    /// Every key the app stores, as a type rather than as a list of constants.
    ///
    ...
    /// it is, and the app rebinds to the default location, so the old note is one
    ...
    ///     domain whatever `BIRTA_MAC_DEFAULTS_SUITE` says, so a reset that
    ...
    /// Where AppKit keeps `setFrameAutosaveName("AppPanel")`'s frame. Spelled
    /// out because `reset` has to remove a key nothing else here writes.
    static let panelFrameAutosaveDefaultsKey = "NSWindow Frame AppPanel"
    ...
    /// `BIRTA_MAC_SCRATCHPAD` overrides it for a run, so mac/scripts/measure.sh
    /// can type into a throwaway file and never touch the real one.
    static var scratchpadURL: URL! {
        get {
            if let env = ProcessInfo.processInfo.environment["BIRTA_MAC_SCRATCHPAD"], !env.isEmpty { return URL(fileURLWithPath: env) }
    ...
    /// When set, the app edits this document instead of the scratchpad.
    ...
    /// and the scratchpad in precedence: a document the user pointed the app at
    /// outranks it, and it outranks the scratchpad, which is where the app starts
    ...
    static var currentNoteURL: URL? {
        get {
            ...
            // A note deleted from Finder must not leave the app bound to nothing.
    ...
    ///
    /// The precedence lives in `BirtaWriterCore.ActiveBinding`, which also
    ...
    /// Write while you type. Off means the app stops writing on edits and nothing
    /// else: hiding the panel and quitting still write, because a preference
    /// that drops the buffer is not one anybody asked for
    /// (`BirtaWriterCore.AutosavePolicy` holds that rule and its tests).
    ...
    /// menu-bar accessory is what the app is; the Dock icon is for somebody who
    ...
    /// bundle out of `mac/build/` against a throwaway domain, so every run is
    ...
    /// litter that `mac/scripts/reap.sh` cannot see: a login item lives in
    ...
    /// nothing sets `BIRTA_MAC_DEFAULTS_SUITE` there and the runner's own
    ...
    /// `mac/scripts/reap.sh` cannot see it and cannot clear it.
    static func applyOnboardingDefaults(firstLaunch: Bool = isFirstLaunch,
        ...
        // `mac/scripts/measure.sh` reads this.
        if ProcessInfo.processInfo.environment["BIRTA_MAC_MEASURE"] == "1" {
            FileHandle.standardError.write(Data(
                "birta-trace onboarding loginitem=\(apply ? "registered" : "skipped")\n".utf8))
        }
    ...
    /// often the panel is summoned. This is a menu-bar app that stays running
    ...
    /// somebody who wants no link previews also gets no fixes, and the app is on
    ...
    /// Where a new note goes: beside the scratchpad, in the app's own folder. Not
    /// a setting of its own, because the scratchpad's location already answers
    /// "where does the app keep things" and two answers would disagree.
    ...
    /// chose and to `BIRTA_MAC_SCRATCHPAD`. This one is spelled from the
    ...
    static func bootConfig(viewStateFor url: URL) -> BootConfig {
        BootConfig(
            ...
            // `HOST_PROFILES.mac`, and a list built by appending would give
