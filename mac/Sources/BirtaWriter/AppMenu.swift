...
/// Every menu the app builds, as one table.
///
...
/// `shared/__tests__/menuChordParity.test.ts` reads the table to hold the app's
...
///
/// Only the app's OWN actions. Undo, Cut, Copy, Paste, Select All, Close and
...
/// for that same command in `package.json`. The app ships zero behavior Birta
...
enum AppMenu {
    ...
    struct Row {
        ...
        /// would already be ambiguous to the builder. `AppMenuTests` pins that
        /// they do not.
        var itemIdentifier: NSUserInterfaceItemIdentifier {
            NSUserInterfaceItemIdentifier(
                "com.birtalabs.birta-writer.menu.\(menu.rawValue)/\(submenu ?? "")/\(title)")
        }
    ...
    /// `menuChordParity.test.ts` enforces by asking `hostHasCommand` under the
    /// mac profile. The cost of the other error is quieter and is what these
    ...
    ///
    /// The zoom chords are the one place the Mac app deliberately parts from the
