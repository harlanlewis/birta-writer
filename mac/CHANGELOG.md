...
- A first launch registers Birta Writer for Mac to start with your Mac again. Since the notes folder began following the product name, the launch recorded that folder before it asked whether this was a first launch, so the answer was always no and the login item was never registered, while the tour said it had been. The reading is taken before anything is stored now. This reaches only an install whose first launch was on a release since 2026.826.0; an install from before, or one that has since turned Start at login on by hand, is unaffected.
