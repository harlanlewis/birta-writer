...
- Showing and hiding the file list is now a button in the titlebar, just after the traffic lights and before the file's name, drawn as the same pane mark the table of contents' button carries at the other end of the band and mirrored to point the other way. The toolbar no longer carries one, and with it goes the preview that flew the list out while the pointer rested on that button: the titlebar button shows and hides the list, and ⇧⌘E and View > Show Files still do. A window on a single file has no file list, draws no button and reserves no room for one, so its name starts against the traffic lights.
