...
- A built-in theme can be removed, and it stays removed. Settings, Appearance, Add Theme, Restore Built-in Themes puts back the built-in themes that are no longer in your library and changes nothing else: the themes you added are untouched, a built-in theme still there is left as it is, and no theme is switched. With nothing missing, it says so.
- A theme you import that happens to share a built-in theme's name now stands beside it rather than replacing it. Both are listed and each is removed on its own, and the built-in one says so in its name for as long as the two share it.
...
- Birta Writer for Mac is signed with an Apple Developer ID and notarized by Apple, so macOS can say who built it. A release downloaded from GitHub opens the way any other Mac app does, with no warning to dismiss and nothing to turn off, and the check works with no network because the notarization ticket travels inside the app. Until now the app was ad-hoc signed, which macOS cannot attribute to anyone, and getting it open meant stripping the download quarantine first: that is why it was only ever offered to people who also had the source.
- `mac/scripts/update.sh` no longer strips the download quarantine. It asks Gatekeeper about the release it just fetched and prints the answer, then installs with the quarantine left in place, because that flag is what makes macOS check and a build that survives the check gains nothing from dodging it. A release cut before signing is refused rather than installed, naming what was asked and what came back; `BIRTA_ALLOW_UNSIGNED=1` installs one anyway for a machine whose owner also owns the source.
- Choosing Claude Code or Codex CLI from the AI Agent pull-down writes a command that narrates itself. The line in the corner of a running `/ai` request is read out of the structured events a harness prints, and the commands Birta Writer for Mac offered asked for none, so a run said nothing until it finished and then said one line. Claude Code now gets `--output-format stream-json --verbose` and Codex `--json`, which is what Birta Writer for VS Code has offered all along, and the corner names each step as the run works: what it is reading, what it is editing, that it is thinking. A command you have already got is left alone; choose your tool from the pull-down again to take the new one, and the pull-down still names your tool either way.
- The Test button in Settings, AI Agent shows what your tool said rather than what it printed. A command that asks for structured events answers in events, so the box under "It works!" filled with JSON wrapped around a single sentence. It now shows the run as a person reads it, one line per step and the answer in full, and a command that answers in plain words reaches the box exactly as it printed. A failure still shows the tool's own error, which is the thing that says what to fix.
- Turning `/ai` on or off leaves the editor where it is. Settings, AI Agent, Enable /ai commands used to rebuild the page underneath you, so the note you were reading blinked out and came back, and only the front window was rebuilt. Birta Writer for Mac now hands every open window what it provides, live, and the slash row and the command palette follow it without the document moving.
- Two settings rows have moved. Rich link previews and embeds is at the foot of Settings, Markdown, with the rest of what decides how a document is drawn, and Open from Terminal is in Settings, Advanced, because installing a command writes a file somewhere else on your Mac rather than storing a preference. The field naming that command is under the switch that installs it and appears with the command, so a switch that is off is a single row; it stays while an install has been refused, because the name is usually what a refusal is about.
...
- An agent's own words survive an odd byte and a hang. One byte that is not UTF-8 anywhere in what your agent command printed, such as a filename that is not, used to empty the Settings Test box and replace a failed run's message with "exited with status 4"; the text now comes through with that byte repaired. A command that says something and then hangs shows what it said beside the sentence saying Birta Writer for Mac stopped waiting, instead of the sentence alone.
- A file changed by another program while Birta Writer for Mac has it open is no longer written over. The app read the file it was bound to once and never again, so an edit made in another editor, or by `git checkout` or a formatter, was replaced by this window's copy on the next hide or Cmd+S, and with autosave on you did not have to have typed anything for that to happen. The app now looks at the file before every write and at every summon: when this window has no unsaved edits it re-reads and shows you the new version, and when both have changed it writes nothing and asks, Reload from Disk against Keep My Changes. A window closed or quit before that question is answered keeps its own text beside the file as `<name> (unsaved).md`, or in your notes folder if that one will not take it, and says where it went the next time you summon the panel. A file nothing has touched is still left alone, bytes, mtime and all.
...
- The formatting toolbar in Birta Writer for Mac is turned on in Settings > Appearance > Formatting toolbar, and from Formatting toolbar in the toolbar's gear menu. The T button that opened it from the title bar is gone. The setting is off by default, as the row was, and either switch still applies to every window and tab at once. Whether you want a row of formatting controls is a question asked once, and a button on the bar of every window was spending permanent space on it; the controls it holds are in the Format menu, the slash menu and the command palette whether the row is showing or not.
- Showing and hiding the file list is now a button in the titlebar, just after the traffic lights and before the file's name, drawn as the same pane mark the table of contents' button carries at the other end of the band and mirrored to point the other way. It inks its pane while the list is out, as that button does. The toolbar no longer carries one, and with it goes the preview that flew the list out while the pointer rested on that button: the titlebar button shows and hides the list, and ⇧⌘E and View > Show Files still do. A window on a single file has no file list, draws no button and reserves no room for one, so its name starts against the traffic lights.
...
- At rest, a window puts away everything it draws over the document, and puts it all away together. Resting means the window has neither the pointer nor the keyboard, and what goes is the search, gear and sidebar buttons in the titlebar band, the formatting row under it when it is showing, and the palette that floats over a selection, on one fade with the titlebar's own buttons. The formatting row and the selection palette used to stay lit on a window nobody was looking at. Nothing moves: every control keeps its box, so the text below is where it was and comes back to where it was.
- A submenu in the toolbar's menus opens by resting on its row, as a menu does everywhere else on macOS. Opening the menu itself still takes a click.
- The toolbar's gear menu draws fewer rules. A rule between two groups that are each a single row was separating two rows rather than two groups, so the keyboard cheatsheet now sits with Birta Writer Settings and the formatting row's switch with Proofreading. The menu's groups themselves are unchanged, and a menu that has whole groups to separate keeps every rule it had.

### Fixed

- The text ran under the file list, and under the table of contents' drawer, whenever a window opened with one of them showing: a docked drawer covered the start of every line, or the end of it. The window's content width never reached the page at launch, and without it the text is laid out as though nothing were docked beside it. It is also why a stored Fixed width did nothing until the control was touched. The width is put on the page now, before the first line is drawn.
- The window's controls faded out from one end while the path popup its file name opens was still on screen. Opening that popup gives up the window's keyboard, and only the titlebar's own half of the band knew to hold its controls through it; the bar below them went quiet under an open popup. Both halves hold now, and both let go when it closes.
- Window > Minimize and Window > Zoom draw the symbols macOS gives them, as Fill and Center beside them already did. The app clears the symbols macOS adds to menu rows, because one decorated row among plain ones reads as a mistake; the Window menu is half the system's, and its rows keep their symbols whatever the app does, so that clearing was producing the mixed menu it exists to prevent.
...
- An Appearance pane in Settings, and VS Code color themes. Birta Writer for Mac follows the macOS light and dark appearance as before, on an Auto light/dark mode switch that is on by default. On, each of light and dark has a theme: macOS Light or macOS Dark, the app's own palettes, or any VS Code color theme, so daytime can be one theme and night another, or a theme by day and macOS dark by night. Off, there is one theme, and it holds whichever of light and dark it is; the switch remembers both arrangements, so turning it off brings back the theme last held and turning it on brings back both slots. Themes are added from a theme's `.json` file, a theme extension's folder, a `.vsix`, a list of every theme installed in or shipped with VS Code, Cursor and their siblings to tick the ones wanted, or by browsing the Open VSX registry from the pane, which opens onto its most downloaded themes; a theme is removed from the minus in its card's corner, with no confirmation. A picker lists the themes of its own mode first, so the dark themes lead in the picker for dark and the light ones in the picker for light. A theme colours the whole page and its code blocks, its own token colours included, and every window and tab follows a change at once, chrome included. The theme in force is also picked under View > Theme and in the command palette, where the modes and every theme are rows and the ones in force say so. Beside the themes, the pane carries an accent and a paper tint, which recolour the system's palette or a theme on the fly, a switch for each drawer's ground, and the font and font size the toolbar's menu already sets, for every window at once. Each drawer keeps the ground it has and moves on its own: the file list is a surface set into the window until Transparent file list sidebar is turned on, and the table of contents reads as part of the page until Transparent table of contents sidebar is turned off, which gives it the file list's shaded card.
...
- Right-clicking a row of the explorer opens a menu of its own in place of the browser's text menu: Open in New Tab, Reveal in Finder, Copy Path and Move to Trash for a note (a file the editor hands to another application gets the same menu without Open in New Tab), and New Note in that folder, Reveal in Finder and Copy Path for a folder.
...
- Folders. Birta Writer for Mac opens a folder as a window with a file explorer beside the editor: Open (⌘O) accepts a folder, and so do Open With in the Finder, a drop on the Dock icon and `open -a`. The window opens on the most recently used note anywhere inside the folder, or the newest at its top level, or a new note when it has none. The explorer lists every file in the folder and below it, folders first, one level at a time as you open them, with files the editor does not open dimmed and opening in their own app; Show Hidden Files (⇧⌘.) lists dotfiles, and Show Files (⇧⌘E) hides and shows the explorer. Both rows are live only in a folder window. New Note and New Tab in a folder window make the note in the folder, as a tab, and a file under the folder opened from anywhere else joins that window as a tab and is selected in its explorer. Changes on disk reach the explorer as they happen. The explorer itself is the editor's, so its look and keyboard are the table of contents drawer's; its own entry is in the editor's changelog.
- A command palette. Command Palette (⇧⌘P) in the View menu, or the ⌘ button in the titlebar after Open Recent, opens one field over everything Birta Writer for Mac can do: every menu row with its chord beside it, the editor's commands that no menu names, the other open windows and tabs, every Settings row, and the files the window can reach. Type a few letters in order and the best match ranks first (`ital` finds Italic, `sca` finds Save a Copy As), the letters that matched are drawn heavier, rows you pick often rise, and a menu's submenu or a Settings pane opens as a level of its own with Backspace on an empty field going back up. Picking a window brings it forward, picking a Settings row opens Settings on it, and picking a file opens it the way the Finder would. Go to File (⌘P) in the File menu is the same palette over files alone: in a folder window it lists the files under the folder by name and path (a very large tree is listed up to a cap, and the palette says when it stopped), elsewhere the notes folder and the files opened lately. ⌘P and ⇧⌘P switch between the two while the palette is up.
