...
///
/// The app's payload differs from the extension's in exactly one way, and it is
/// the caller's rather than this type's: the path handed in is absolute,
/// because the app's file lives under Application Support and a workspace-relative
...
final class AgentReferenceTests: XCTestCase {
    ...
    func testAnAbsolutePathIsCarriedThroughUnchanged() {
        // The app's whole difference from the extension, and it is nothing this
        // type does: it references whatever path it is handed.
        let path = "/Users/x/Documents/Birta Writer/Birta Writer.md"
    ...
    func testStaleCoordinatesPastTheEndFallBackToTheReferenceAlone() {
        ...
        // has and the Mac app does not: the page sends the span, and the source is the
        // buffer the app already holds. A reference alone is the honest answer
    ...
    func testALongSelectionIsCopiedWhole() {
        ...
        // which the app has no equivalent of. Someone who selected a long passage
        // and asked for it meant it, and the app's Copy Everything has no ceiling
        // for the same reason.
