...
/// What a move of the app's notes would do, decided with no disk in the room.
///
...
final class NoteMoveTests: XCTestCase {
    ...
    /// The folder is shared, so a file the app did not put there stays put.
    ...
    func testANoteWhoseNameIsTakenShouldBeNumberedRatherThanOverwrite() {
        let plan = NoteMove.plan(
            source: source, destination: destination,
            entries: [url("/old/notes/Note.md")],
            attachments: [],
            exists: { $0.path == "/new/notes/Note.md" })

        XCTAssertEqual(plan.items.first?.destination.path, "/new/notes/Note 2.md")
    }

    /// The numbering keeps counting past a run of taken names.
    func testNumberingShouldSkipEveryNameAlreadyTaken() {
        let taken: Set<String> = ["/new/notes/Note.md", "/new/notes/Note 2.md", "/new/notes/Note 3.md"]
        let plan = NoteMove.plan(
            source: source, destination: destination,
            entries: [url("/old/notes/Note.md")],
            attachments: [],
        ...
        XCTAssertEqual(plan.items.first?.destination.path, "/new/notes/Note 4.md")
    }
    ...
    func testTwoNotesLandingOnOneNameShouldNotBothGetIt() {
        ...
        // `Note.md`, so the first becomes `Note 2.md` and the second must not.
        let plan = NoteMove.plan(
            source: source, destination: destination,
            entries: [url("/old/notes/Note.md"), url("/old/a/Note.md")],
            attachments: [],
            exists: { $0.path == "/new/notes/Note.md" })
        ...
        XCTAssertEqual(landings, ["/new/notes/Note 2.md", "/new/notes/Note 3.md"])
    }
