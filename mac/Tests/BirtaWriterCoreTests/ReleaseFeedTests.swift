...
final class ReleaseFeedTests: XCTestCase {
    ...
    func testTheAppArchiveAndItsChecksumShouldBothBeFound() {
        let release = ReleaseFeed.parse(json(tag: "v2026.820.0",
                                             assets: ["BirtaWriter-2026.820.0.zip",
                                                      "BirtaWriter-2026.820.0.zip.sha256"]))
        XCTAssertEqual(release?.tag, "v2026.820.0")
        XCTAssertEqual(release?.appURL.lastPathComponent, "BirtaWriter-2026.820.0.zip")
        XCTAssertEqual(release?.checksumURL?.lastPathComponent, "BirtaWriter-2026.820.0.zip.sha256")
    }
    ...
    func testTheChecksumShouldNeverBeMistakenForTheArchive() {
        let checksumFirst = ReleaseFeed.parse(json(tag: "v1.0.0",
                                                   assets: ["BirtaWriter-1.0.0.zip.sha256",
                                                            "BirtaWriter-1.0.0.zip"]))
        XCTAssertEqual(checksumFirst?.appURL.lastPathComponent, "BirtaWriter-1.0.0.zip")
        XCTAssertEqual(checksumFirst?.checksumURL?.lastPathComponent, "BirtaWriter-1.0.0.zip.sha256")
    }
    ...
    func testAnArchiveWithNoChecksumShouldStillBeUsable() {
        let release = ReleaseFeed.parse(json(tag: "v1.0.0", assets: ["BirtaWriter-1.0.0.zip"]))
