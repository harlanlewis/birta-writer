...
final class StatusOverlayInkTests: XCTestCase {
    ...
    ///
    /// `StatusOverlay` lives in the `BirtaWriter` app target, which has no test
    /// target: `BirtaWriterCore` is deliberately AppKit-free so it can be tested
    ...
    func testTheOverlayShouldStillUseTheColoursThisFileMeasures() {
        let source = URL(fileURLWithPath: #filePath)
            ...
            .deletingLastPathComponent()
            .deletingLastPathComponent()
            .appendingPathComponent("Sources/BirtaWriter/StatusOverlay.swift")
