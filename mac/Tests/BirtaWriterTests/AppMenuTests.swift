...
final class AppMenuTests: XCTestCase {
    ...
    /// A built menu, for one of the app's own menus.
    private func build(_ menu: AppMenu.Menu) -> NSMenu {
        let nsMenu = NSMenu(title: menu.rawValue)
        AppMenu.add(menu, to: nsMenu, target: self)
        return nsMenu
    }
    ...
    ///
    /// `shared/__tests__/macAbout.test.ts` holds the same claim by reading the
    ...
    /// what opens the app's own About window instead of AppKit's standard panel.
    ...
    func testASubmenuRowShouldOpenItsSubmenuAndRouteNothingElse() {
        ...
        XCTAssertEqual(seen, AppMenu.rows.filter { $0.action.opensSubmenu }.count,
    ...
    /// by asking `hostHasCommand` under the mac profile; this is the positive
    /// half.
    ///
    ...
    func testTheViewMenuShouldOfferTheTableOfContents() {
        ...
        XCTAssertEqual((item?.representedObject as? AppMenu.Command)?.id, "toggleToc")
    ...
    func testTheFileMenuShouldOpenRecentThroughASubmenuOfItsOwn() {
        ...
        XCTAssertEqual(item?.submenu?.identifier, AppMenu.recentsMenuIdentifier)
    ...
    func testTheRecentsRowShouldBeReachableBySelectorForTheTitlebarButton() {
        ...
        let row = AppMenu.row(for: #selector(AppDelegate.menuOpenRecent(_:)))
        XCTAssertEqual(row?.title, "Open Recent")
        XCTAssertNil(AppMenu.row(for: #selector(AppDelegate.menuRunEditorCommand(_:))),
    ...
    /// asking `hostHasCommand` under the mac profile. And the submenu is
    ...
    func testTheStyleOptionsSubmenuShouldOfferEveryCategoryThePageToggles() throws {
        ...
        for (row, category) in zip(rows, StyleCategory.allCases) {
            let command = row.representedObject as? AppMenu.Command
    ...
    func testEveryStatefulRowShouldDrawTheStateItIsGiven() throws {
        ...
        AppMenu.applyState(MenuState(proofreadOptions: ["spellCheck": false, "fillers": false],
                                     noteHighlight: false,
                                     tocShown: true),
                           to: view)
        ...
        // above.
        AppMenu.applyState(MenuState(proofreadOptions: ["spellCheck": true, "fillers": true],
                                     noteHighlight: true,
                                     tocShown: false),
                           to: view)
    ...
    func testTheRepaintShouldReachEveryRowThatDeclaresAState() {
        ...
        let stateless = AppMenu.rows.filter { $0.menu == .view && $0.state == nil && $0.action.command != nil }
        XCTAssertGreaterThan(stateless.count, 5)
        for statelessRow in stateless {
            let item = items.first { ($0.representedObject as? AppMenu.Command) == statelessRow.action.command }
    ...
    func testTheGateShouldWithdrawWhatItGovernsRatherThanLeaveItSetAndDead() throws {
        ...
        AppMenu.applyState(MenuState(proofreadOptions: ["proofreading": false]), to: view)
        ...
        AppMenu.applyState(MenuState(proofreadOptions: ["proofreading": true]), to: view)
    ...
    func testTurningOffCheckStyleShouldWithdrawTheStyleOptions() throws {
        ...
        AppMenu.applyState(MenuState(proofreadOptions: ["styleCheck": false]), to: view)
        XCTAssertEqual(visibleTitles(of: proofreading), [
            "Proofreading",
    ...
    func testNoTwoRowsShouldShareAnItemIdentifier() {
        ...
        XCTAssertEqual(seen.count, AppMenu.rows.count)
        XCTAssertGreaterThan(seen.count, 60)
    }

    private func row(_ items: [NSMenuItem], _ id: String, arg: String? = nil) -> NSMenuItem? {
        items.first { ($0.representedObject as? AppMenu.Command) == AppMenu.Command(id, arg: arg) }
    }
    ...
    func testEveryCommandRowShouldCarryItsCommandIdToOneRouter() {
        ...
        for menu in AppMenu.Menu.allCases {
            for item in allItems(of: build(menu)) where item.action == router {
                guard let command = item.representedObject as? AppMenu.Command else {
    ...
    ///
    /// `AppMenu.applyState` finds a row by what its item carries, so a
    ...
    func testNoTwoRowsShouldRunTheSameCommand() {
        var seen: [AppMenu.Command: String] = [:]
        var counted = 0
        for row in AppMenu.rows {
    ...
    func testTheDeclaredShortcutsShouldBeExactlyTheRowsThatBindAKey() {
        let keyed = AppMenu.rows.filter { !$0.key.isEmpty }
        XCTAssertEqual(AppMenu.shortcuts.count, keyed.count)
        XCTAssertGreaterThan(AppMenu.shortcuts.count, 20)
        // A row with no key is a menu row, not a shortcut: printing it would
        // put a blank key column in the cheatsheet.
        XCTAssertFalse(AppMenu.shortcuts.contains { $0.keys.isEmpty })
        for (shortcut, row) in zip(AppMenu.shortcuts, keyed) {
    ...
    func testACommandRowShouldDeclareItsCommandAndAnAppRowShouldNot() {
        let link = AppMenu.shortcuts.first { $0.label == "Link…" }
        ...
        XCTAssertNil(AppMenu.shortcuts.first { $0.label == "Save" }?.command)
    }

    // MARK: rows a button repeats

    func testFileShouldOfferOpenOnTheConventionalChord() {
        let open = AppMenu.rows.first { $0.title == "Open…" }
    ...
    func testTheSelectorLookupShouldRefuseTheCommandRowsSharedSelector() {
        ...
        XCTAssertNil(AppMenu.row(for: #selector(AppDelegate.menuRunEditorCommand(_:))))
        XCTAssertNil(AppMenu.row(for: #selector(AppDelegate.menuOpenAbout)))
    }

    // MARK: the Window menu

    func testTheWindowMenuShouldAuthorTheAppsOwnRowsAndNoneOfTheSystems() {
        let window = AppMenu.windowMenu()
    ...
    func testNoTwoRowsShouldBindTheSameChord() {
        ...
        for row in AppMenu.rows where !row.key.isEmpty {
            let chord = row.chord
