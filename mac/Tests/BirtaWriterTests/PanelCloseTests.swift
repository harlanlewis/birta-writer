...
///
/// The close button and Cmd+W both reach `AppPanel.close`, and the app decides:
...
/// `AppPanel` carries that as a request while anybody is listening for one, and
...
final class PanelCloseTests: XCTestCase {
    ...
    /// app's standard defaults whatever `BIRTA_MAC_DEFAULTS_SUITE` says, so a
