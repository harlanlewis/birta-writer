...
/// `birta-trace onboarding loginitem=` line that `mac/scripts/measure.sh` greps.
...
/// `BIRTA_MAC_DEFAULTS_SUITE`, and the runner's own standard domain holds none
...
/// than in a plist under our domain, so `mac/scripts/reap.sh` cannot see it
...
final class OnboardingDefaultsTests: XCTestCase {
    ...
    ///
    /// This is the arm `isUserStore` exists for. `mac/scripts/measure.sh`
    /// launches a bundle out of `mac/build/` against a throwaway domain, so
    ...
    ///
    /// `hasSeenWelcome` is absent for everybody who had the app before that screen
