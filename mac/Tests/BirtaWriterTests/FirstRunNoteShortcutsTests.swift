...
final class FirstRunNoteShortcutsTests: XCTestCase {
    ...
    ///
    /// Asked of `AppMenu.rows`, the whole table, rather than of the keyed
    ...
    private func rowsBinding(_ chord: (shift: Bool, key: String)) -> [AppMenu.Row] {
        let named: NSEvent.ModifierFlags = chord.shift ? [.command, .shift] : [.command]
        return AppMenu.rows.filter { row in
    ...
    func testEveryChordTheTourNamesShouldBeBoundInTheMenu() {
        ...
        for chord in chords {
            ...
            XCTAssertEqual(rows.count, 1,
                           "the tour names \(spelling); AppMenu.rows binds it \(rows.count) times"
    ...
    func testAChordNothingBindsShouldNotMatch() {
        let bound = AppMenu.rows.contains { row in
