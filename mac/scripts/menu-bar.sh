...
#
# The one thing no test in `mac/Tests` can see. A menu bar is drawn by the
...
# system's sitting among them. So `AppMenuTests` asserts the half that is ours
# and this reads the half that is not, and it is also the only place the
# checkmarks `AppMenu.applyState` writes can be seen as a person sees them.
#
# Usage: bash mac/scripts/menu-bar.sh [menu name ...]
#   bash mac/scripts/menu-bar.sh              # every menu but Apple's
#   bash mac/scripts/menu-bar.sh View Window  # just those
#
...
WORK="$(mktemp -d -t mac-menubar)"
export BIRTA_MAC_SCRATCHPAD="$WORK/Scratch pad.md"
export BIRTA_MAC_DEFAULTS_SUITE="com.birtalabs.birta-writer.menubar.$$"
LOG="$(mktemp -t mac-menubar-log)"
...
# the plist goes too, BY EXACT NAME: a glob over `com.birtalabs.birta-writer.*` would
# take the user's real settings, since the app's own domain is a prefix of
# every throwaway one.
trap 'end_app; rm -rf "$WORK"; rm -f "$LOG"; defaults delete "$BIRTA_MAC_DEFAULTS_SUITE" >/dev/null 2>&1 || true; rm -f "$HOME/Library/Preferences/$BIRTA_MAC_DEFAULTS_SUITE.plist"' EXIT
...
# identifier `com.birtalabs.birta-writer`. System Events indexes processes by that
...
BIRTA_MAC_MEASURE=1 BIRTA_MAC_SHOW_ON_LAUNCH=1 "$APP" 2>"$LOG" &
PID=$!

n=0
while ! grep -q "^mac-measure ready " "$LOG" 2>/dev/null; do
    sleep 0.2; n=$((n + 1))
