...
# built app under BIRTA_MAC_MEASURE=1 and drives it through the debug signals
...
#            its type is __birtaKeys, type its keys into the panel as NSEvents
...
# answer.
#
# Usage: bash mac/scripts/measure.sh [--keep] (keep leaves the app running,
...
SCRATCH_DIR="$(mktemp -d -t mac-measure-scratch)"
...
export BIRTA_MAC_SCRATCHPAD="$SCRATCH_DIR/Scratch pad.md"
...
# Removed by EXACT name, never by a glob over `com.birtalabs.birta-writer.*`: the app's
...
EXTRA_SUITES=""
trap '[ $KEEP = 1 ] || end_app; rm -rf "$SCRATCH_DIR"; for s in $BIRTA_MAC_DEFAULTS_SUITE $EXTRA_SUITES; do defaults delete "$s" >/dev/null 2>&1 || true; rm -f "$HOME/Library/Preferences/$s.plist"; done' EXIT
...
last() { grep "^mac-measure $1 " "$LOG" | tail -1 | awk '{print $3}'; }
...
show_panel
printf '{"type":"__birtaKeys","keys":["End","Enter","Enter","N","e","x","t","Enter","l","i","n","e"]}' > "$SCRATCH_DIR/.debug-message.json"
...
# before one exists: the table is parsed by `shared/__tests__/appMenuTable.ts`,
# the built NSMenus are read back by `AppMenuTests`, and the chords are compared
...
show_panel
printf '{"type":"__birtaKeys","keys":["End","Enter","Enter","B","u","l","l"]}' > "$SCRATCH_DIR/.debug-message.json"
kill -URG $PID; sleep 2
printf '{"type":"__birtaKeys","keys":["cmd+shift+8"]}' > "$SCRATCH_DIR/.debug-message.json"
kill -URG $PID; sleep 2
hide_panel
...
CLIP_BACKUP="$(mktemp -t mac-measure-clip)"
...
printf '{"type":"__birtaKeys","keys":["cmd+v"]}' > "$SCRATCH_DIR/.debug-message.json"
...
MARKS_AT_LAUNCH="$(grep -c "birta-trace markfolders " "$LOG" || true)"
...
printf '{"type":"__birtaSaveNow"}' > "$SCRATCH_DIR/.debug-message.json"
...
if ! grep -q "^birta-trace noteMissing " "$LOG"; then
    ...
    grep -E "birta-trace (writeattempt|noteMissing)|mac-measure (visible|hide)" "$LOG" | tail -20 >&2; exit 1
fi
...
# contributes nothing to the PDF path `__birtaSnapshot` uses, so a picture of this
# state cannot show the page's controls whether they are there or not.
MISSING="$(grep "^birta-trace missingscreen " "$LOG" | tail -1)"
case "$MISSING" in
    *"webviewHidden=false"*) ;;
...
show_panel
printf '{"type":"__birtaHoverButton","index":0,"hovered":true}' > "$SCRATCH_DIR/.debug-message.json"
kill -URG $PID; sleep 1.2
rm -f "$SCRATCH_DIR/.debug-message.json"
M_TIP="$(grep "^birta-trace hovertooltip " "$LOG" | tail -1 | sed 's/^birta-trace hovertooltip //')"
...
printf '{"type":"__birtaHoverButton","index":0,"hovered":false}' > "$SCRATCH_DIR/.debug-message.json"
...
M_PAGE_COUNT="$(grep "^birta-trace titlebarstrip " "$LOG" | tail -1 | sed -n 's/.*pageCount=\([0-9]*\).*/\1/p')"
if [ "${M_PAGE_COUNT:-0}" != "1" ]; then
    echo "missing screen       FAILED: the band holds ${M_PAGE_COUNT:-no} page controls, wanted the gear alone" >&2
    grep "^birta-trace titlebarstrip " "$LOG" | tail -1 | sed 's/^/  /' >&2; exit 1
fi
...
printf '{"type":"__birtaEditorLock"}' > "$SCRATCH_DIR/.debug-message.json"
kill -URG $PID; sleep 1.2
rm -f "$SCRATCH_DIR/.debug-message.json"
LOCK="$(grep "^birta-trace editorlock " "$LOG" | tail -1 | sed 's/^birta-trace editorlock //')"
case "$LOCK" in
...
printf '{"type":"__birtaResting"}' > "$SCRATCH_DIR/.debug-message.json"
kill -URG $PID; sleep 1.2
rm -f "$SCRATCH_DIR/.debug-message.json"
REST="$(grep "^birta-trace restingchrome " "$LOG" | tail -1 | sed 's/^birta-trace restingchrome //')"
case "$REST" in
...
printf '{"type":"__birtaReload"}' > "$SCRATCH_DIR/.debug-message.json"
kill -URG $PID
...
R_PAGE_COUNT="$(grep "^birta-trace titlebarstrip " "$LOG" | tail -1 | sed -n 's/.*pageCount=\([0-9]*\).*/\1/p')"
...
show_panel
printf '{"type":"__birtaSaveMissingBack"}' > "$SCRATCH_DIR/.debug-message.json"
kill -URG $PID; sleep 1.2
rm -f "$SCRATCH_DIR/.debug-message.json"
if [ ! -e "$SCRATCH_DIR/Scratch pad.md" ]; then
...
kill -URG $PID; sleep 1.2
rm -f "$SCRATCH_DIR/.debug-message.json"
LOCK_BACK="$(grep "^birta-trace editorlock " "$LOG" | tail -1 | sed 's/^birta-trace editorlock //')"
case "$LOCK_BACK" in
...
kill -URG $PID; sleep 1.2
rm -f "$SCRATCH_DIR/.debug-message.json"
REST_BACK="$(grep "^birta-trace restingchrome " "$LOG" | tail -1 | sed 's/^birta-trace restingchrome //')"
case "$REST_BACK" in
...
for attempt in 1 2 3; do
    printf '{"type":"__birtaKeys","keys":["Escape","End","Enter","/","d","a","t","e"]}' > "$SCRATCH_DIR/.debug-message.json"
    kill -URG $PID; sleep 1.5
    rm -f "$SCRATCH_DIR/.debug-message.json"
    printf '{"type":"__birtaKeys","keys":["Enter"]}' > "$SCRATCH_DIR/.debug-message.json"
    kill -URG $PID; sleep 1.5
    rm -f "$SCRATCH_DIR/.debug-message.json"
    DATEPICK="$(grep "^birta-trace datepicker " "$LOG" | tail -1 || true)"
    [ -n "$DATEPICK" ] && break
    # Dismiss whatever did land, and ask for the window again.
    printf '{"type":"__birtaKeys","keys":["Escape"]}' > "$SCRATCH_DIR/.debug-message.json"
    kill -URG $PID; sleep 0.5
    rm -f "$SCRATCH_DIR/.debug-message.json"
    show_panel
done
...
# `birta-trace titlebar` line (Coordinator.traceTitleBar) and this reads it.
TITLEBAR="$(grep "^birta-trace titlebar " "$LOG" | tail -1 || true)"
...
# of them agreed the title was fine while the window drew only its first word:
...
# can see.
CHEV="$(grep "^birta-trace chevron " "$LOG" | tail -1 || true)"
...
ACTS="$(grep "^birta-trace titleactions " "$LOG" | tail -1 || true)"
...
AC_WANT="$(awk '/static let shipped: \[Action\] = \[/{f=1;next} f&&/^ *\]/{exit} f&&/\.init\(selector:/{n++} END{print n+0}' \
    mac/Sources/BirtaWriter/TitlebarActions.swift)"
if [ -z "$ACTS" ]; then
...
# that looks like.
STRIP="$(grep "^birta-trace titlebarstrip " "$LOG" | tail -1 || true)"
...
rm -f "$SCRATCH_DIR/.debug-message.json"
TIP="$(grep "^birta-trace hovertooltip " "$LOG" | tail -1 | sed 's/^birta-trace hovertooltip //')"
...
TIP_BAND="$(grep "^birta-trace titlebarstrip " "$LOG" | tail -1 | sed -n 's/.*nativeMidY=\([0-9.]*\).*/\1/p')"
...
kill -URG $PID; sleep 1.0
rm -f "$SCRATCH_DIR/.debug-message.json"
if [ "$(grep "^birta-trace hovertooltip " "$LOG" | tail -1 | sed 's/^birta-trace hovertooltip //')" = "none" ]; then
    ...
    grep "^birta-trace hovertooltip " "$LOG" | tail -2 | sed 's/^/  /' >&2; exit 1
fi
...
DOCK="$(grep "^birta-trace dock " "$LOG" | tail -1 | sed 's/^birta-trace dock //')"
...
defaults write "$BIRTA_MAC_DEFAULTS_SUITE" autosave -bool NO
show_panel
printf '{"type":"__birtaKeys","keys":["End","Enter","d","i","r","t","y"]}' > "$SCRATCH_DIR/.debug-message.json"
kill -URG $PID; sleep 2
TITLE_DIRTY="$(grep "^birta-trace titletext " "$LOG" | tail -1 | sed 's/^birta-trace titletext //')"
printf '{"type":"__birtaSave"}' > "$SCRATCH_DIR/.debug-message.json"
kill -URG $PID; sleep 2
TITLE_CLEAN="$(grep "^birta-trace titletext " "$LOG" | tail -1 | sed 's/^birta-trace titletext //')"
defaults delete "$BIRTA_MAC_DEFAULTS_SUITE" autosave >/dev/null 2>&1 || true
rm -f "$SCRATCH_DIR/.debug-message.json"
# The save has to have HAPPENED. Without this a `__birtaSave` the app ignored
...
if [ "$(marks debug-save)" -lt 1 ]; then
    echo "edited flag          FAILED: the app never acted on __birtaSave, so nothing was written" >&2
    exit 1
fi
case "$TITLE_DIRTY" in
...
if [ "$EDITED_ROSE" = 1 ] && [ "$EDITED_FELL" = 1 ]; then
    ...
    grep "^birta-trace titletext " "$LOG" | sed 's/^birta-trace titletext /    /' >&2
    echo "  app log: $LOG" >&2
    exit 1
fi
...
show_panel
BURST_START=$(grep -c "^birta-trace titletext " "$LOG")
BURST_WORDS=""
...
for group in 1 2; do
    printf '{"type":"__birtaKeys","keys":["End","Enter","s","t","e","a","d","y"]}' > "$SCRATCH_DIR/.debug-message.json"
    kill -URG $PID; sleep 2.5
    BURST_WORDS="$BURST_WORDS steady"
done
rm -f "$SCRATCH_DIR/.debug-message.json"
BURST="$(grep "^birta-trace titletext " "$LOG" | tail -n +$((BURST_START + 1)) | sed 's/^birta-trace titletext //')"
...
show_panel
DRAG="$(grep "^birta-trace titlebardrag " "$LOG" | tail -1 || true)"
...
DRAG_FIRST="$(grep "^birta-trace titlebardrag " "$LOG" | head -1 || true)"
...
CEIL_WIDE_BEFORE="$(grep "^birta-trace titlebardrag " "$LOG" | tail -1 | sed -n 's/.*windowW=\([0-9.-]*\).*/\1/p')"
printf '{"type":"__birtaRename","name":"%s"}' "$CEIL_NAME" > "$SCRATCH_DIR/.debug-message.json"
kill -URG $PID; sleep 2.5
rm -f "$SCRATCH_DIR/.debug-message.json"
...
ceil_at() { # ceil_at <width>; sets CEIL_* from the traces it provokes
    printf '{"type":"__birtaResize","width":%s}' "$1" > "$SCRATCH_DIR/.debug-message.json"
    kill -URG $PID; sleep 1.2
    rm -f "$SCRATCH_DIR/.debug-message.json"
    local tb drag
    tb="$(grep "^birta-trace titlebar " "$LOG" | tail -1 || true)"
    drag="$(grep "^birta-trace titlebardrag " "$LOG" | tail -1 || true)"
...
printf '{"type":"__birtaRename","name":"Scratch pad.md"}' > "$SCRATCH_DIR/.debug-message.json"
kill -URG $PID; sleep 2.5
rm -f "$SCRATCH_DIR/.debug-message.json"
...
# a second file, and `__birtaRename` is the one way a shell has to give the app
# one without a file chooser or an Accessibility grant.
RECENTS="$(defaults read "$BIRTA_MAC_DEFAULTS_SUITE" recentDocuments 2>/dev/null || true)"
REC_LEFT=0
REC_REBIND=0
...
# chrome and the debug key path reaches the web view, so `__birtaTitleClick` and
# `__birtaRename` are the only way this form is ever built against a real window
...
show_panel
printf '{"type":"__birtaTitleClick"}' > "$SCRATCH_DIR/.debug-message.json"
kill -URG $PID; sleep 1.5
POPOVER="$(grep "^birta-trace titlepopover " "$LOG" | tail -1 | sed 's/^birta-trace titlepopover //')"
rm -f "$SCRATCH_DIR/.debug-message.json"
...
# to be.
RELOCATES_BEFORE=$(grep -c "^birta-trace relocate " "$LOG" || true)
printf '{"type":"__birtaRename","name":"Renamed by measure.md"}' > "$SCRATCH_DIR/.debug-message.json"
kill -URG $PID; sleep 2.5
rm -f "$SCRATCH_DIR/.debug-message.json"
RENAMED="$SCRATCH_DIR/Renamed by measure.md"
TITLE_AFTER="$(grep "^birta-trace titletext " "$LOG" | tail -1 | sed 's/^birta-trace titletext //')"
...
RELOCATES="$(grep "^birta-trace relocate " "$LOG" | tail -n +$((RELOCATES_BEFORE + 1)) | sed 's/^birta-trace relocate //')"
...
CLIP_BACKUP2="$(mktemp -t mac-measure-clip2)"
pbpaste > "$CLIP_BACKUP2" 2>/dev/null || true
show_panel
printf '{"type":"__birtaCopyAgentReference"}' > "$SCRATCH_DIR/.debug-message.json"
kill -URG $PID; sleep 2.5
rm -f "$SCRATCH_DIR/.debug-message.json"
COPIED="$(pbpaste 2>/dev/null || true)"
pbcopy < "$CLIP_BACKUP2" 2>/dev/null || true
rm -f "$CLIP_BACKUP2"
# An ABSOLUTE path, which is the whole difference from the extension: the app's
...
if [ "$REF_OK" = 1 ]; then
    ...
    grep "^birta-trace agentref " "$LOG" | sed 's/^/  /' >&2
    exit 1
fi

# Cold recovery: kill the content process, wait for the remount.
BEFORE=$(grep -c "^mac-measure ready " "$LOG")
WC_AFTER="$(pgrep -f com.apple.WebKit.WebContent | sort || true)"
WC_NEW="$(comm -13 <(printf '%s\n' "$WC_BEFORE") <(printf '%s\n' "$WC_AFTER") | tr '\n' ' ')"
if [ -z "$WC_NEW" ]; then echo "could not identify the app's WebContent process" >&2; exit 1; fi
...
kill -9 $WC_NEW
n=0
while [ "$(grep -c "^mac-measure ready " "$LOG")" -le "$BEFORE" ]; do
    sleep 0.1; n=$((n+1))
    if [ $n -gt 200 ]; then echo "no remount after kill" >&2; cat "$LOG" >&2; exit 1; fi
done
echo "terminate→ready      $(delta "$(last terminate)" "$(last ready)") ms   (cold recovery: WebKit reports the death, the app remounts)"
if grep -q "$STAMP" "$SCRATCH_DIR/Scratch pad.md"; then
...
onboarding_run() { # onboarding_run <suite>
    local dir log pid
    dir="$(mktemp -d -t mac-onboard)"
    log="$(mktemp -t mac-onboard)"
    BIRTA_MAC_MEASURE=1 BIRTA_MAC_SCRATCHPAD="$dir/Onboard.md" \
        BIRTA_MAC_DEFAULTS_SUITE="$1" BIRTA_MAC_OPEN_WELCOME=1 "$APP" 2>"$log" &
    pid=$!
    sleep 3; kill -USR1 $pid; sleep 2
    ONBOARD_KEYS="$(defaults read "$1" 2>/dev/null || true)"
    ONBOARD_LOGIN="$(grep "^birta-trace onboarding " "$log" | tail -1 || true)"
    ONBOARD_ALIVE=0
    grep -q "^mac-measure ready " "$log" && ONBOARD_ALIVE=1
...
#
# The arm therefore MOVES that switch (`BIRTA_MAC_TOGGLE_ICLOUD`) and requires
...
SETTINGS_SUITE="com.birtalabs.birta-writer.measure.settings.$$"
SETTINGS_DIR="$(mktemp -d -t mac-settings)"
SETTINGS_LOG="$(mktemp -t mac-settings)"
BIRTA_MAC_MEASURE=1 BIRTA_MAC_SCRATCHPAD="$SETTINGS_DIR/S.md" \
    BIRTA_MAC_DEFAULTS_SUITE="$SETTINGS_SUITE" BIRTA_MAC_OPEN_SETTINGS=general \
    BIRTA_MAC_TOGGLE_ICLOUD=1 "$APP" 2>"$SETTINGS_LOG" &
SETTINGS_PID=$!
sleep 5
SETTINGS_FITS="$(grep "^birta-trace settingsfit " "$SETTINGS_LOG" || true)"
SETTINGS_TOGGLE="$(grep "^birta-trace icloudtoggle " "$SETTINGS_LOG" | tail -1 || true)"
...
# login item pointing at `mac/build`, which the next checkout replaces, and
...
# ── Autosave off: what the app promises in both directions ──
#
...
#
# `__birtaPrefs` reports and does nothing else, which is the whole reason it
...
for _ in $(seq 1 40); do
    printf '{"type":"__birtaPrefs"}' > "$SCRATCH_DIR/.debug-message.json"
    kill -URG $PID; sleep 0.25
    rm -f "$SCRATCH_DIR/.debug-message.json"
    case "$(grep "^birta-trace prefs " "$LOG" | tail -1)" in
...
if [ "$OFF_SEEN" != 1 ]; then
    ...
    grep "^birta-trace prefs " "$LOG" | tail -3 | sed 's/^/  /' >&2
    exit 1
fi
...
if grep -q "$OFF_STAMP" "$SCRATCH_DIR/Scratch pad.md" 2>/dev/null; then
    ...
    # about the probe.
    grep "^birta-trace writedecision " "$LOG" | tail -3 | sed 's/^/  /' >&2
    ...
    grep "^birta-trace writeattempt " "$LOG" | tail -3 | sed 's/^/  /' >&2
    cat "$SCRATCH_DIR/Scratch pad.md" >&2; exit 1
fi
...
OFF_TITLE="$(grep "^birta-trace titletext " "$LOG" | tail -1 | sed 's/^birta-trace titletext //')"
case "$OFF_TITLE" in
...
# leaves a second one key: `__birtaKeys` types into whichever window is in front,
...
PAD_BEFORE="$(cat "$BIRTA_MAC_SCRATCHPAD" 2>/dev/null || true)"
printf '{"type":"__birtaNewWindow"}' > "$SCRATCH_DIR/.debug-message.json"
kill -URG $PID; sleep 1.2
rm -f "$SCRATCH_DIR/.debug-message.json"
...
for f in "$SCRATCH_DIR"/*.md; do
    [ "$f" = "$BIRTA_MAC_SCRATCHPAD" ] && continue
...
if grep -q "$TWO_STAMP" "$BIRTA_MAC_SCRATCHPAD" 2>/dev/null; then
...
if [ "$(cat "$BIRTA_MAC_SCRATCHPAD" 2>/dev/null || true)" != "$PAD_BEFORE" ]; then
...
#
# Driven through `__birtaOpen`, which skips the chooser and nothing else: the
...
kill -URG $PID; sleep 1.2
rm -f "$SCRATCH_DIR/.debug-message.json"
if ! grep -q "^birta-trace noteMissing at=$(basename "$VACANT_FILE")$" "$LOG"; then
    ...
    grep "^birta-trace noteMissing " "$LOG" | tail -3 | sed 's/^/  /' >&2; exit 1
fi
printf 'a third note\n' > "$SCRATCH_DIR/Third note.md"
printf '{"type":"__birtaOpen","path":"%s"}' "$SCRATCH_DIR/Third note.md" > "$SCRATCH_DIR/.debug-message.json"
...
rm -f "$SCRATCH_DIR/.debug-message.json"
OPENED="$(grep "^birta-trace open " "$LOG" | tail -1 | sed 's/^birta-trace open //')"
...
kill -URG $PID; sleep 1.2
rm -f "$SCRATCH_DIR/.debug-message.json"
if ! grep -q "^birta-trace noteMissing at=Third note.md$" "$LOG"; then
...
printf '{"type":"__birtaOpen","path":"%s"}' "$SCRATCH_DIR/Fourth note.md" > "$SCRATCH_DIR/.debug-message.json"
kill -URG $PID; sleep 1.5
rm -f "$SCRATCH_DIR/.debug-message.json"
KEPT="$(grep "^birta-trace open " "$LOG" | tail -1 | sed 's/^birta-trace open //')"
...
CLOSED="$(grep "^birta-trace closewindow " "$LOG" | tail -1 || true)"
...
RECENTS_BEFORE="$(defaults read "$BIRTA_MAC_DEFAULTS_SUITE" recentDocuments 2>/dev/null || true)"
printf '{"type":"__birtaCloseWindow"}' > "$SCRATCH_DIR/.debug-message.json"
kill -URG $PID; sleep 1.5
rm -f "$SCRATCH_DIR/.debug-message.json"
...
CLOSED_LINE="$(grep "^birta-trace closewindow " "$LOG" | tail -1 || true)"
CLOSED_AT="$(printf '%s' "$CLOSED_LINE" | sed 's/^birta-trace closewindow at=//')"
if [ -z "$CLOSED_AT" ] || [ "$CLOSED_LINE" = "$CLOSED" ]; then
    echo "recents on close     FAILED: no window closed, so nothing below was measured" >&2
    exit 1
fi
RECENTS_AFTER="$(defaults read "$BIRTA_MAC_DEFAULTS_SUITE" recentDocuments 2>/dev/null || true)"
...
MARKS_NOW="$(grep -c "birta-trace markfolders " "$LOG" || true)"
