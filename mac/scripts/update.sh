...
#
#   bash mac/scripts/update.sh              # newest release
...
url_for() { # url_for <suffix>
    printf '%s' "$JSON" \
        | grep -o "https://[^\"]*BirtaWriter-[0-9.]*\.zip$1\"" \
        | head -1 \
        | tr -d '"' \
        || true
}
...
TMP="$(mktemp -d -t mac-update)"
trap 'rm -rf "$TMP"' EXIT
echo "→ downloading $(basename "$ZIP_URL")"
curl -fsSL "$ZIP_URL" -o "$TMP/update.zip"
...
if [ -n "$SUM_URL" ]; then
    curl -fsSL "$SUM_URL" -o "$TMP/update.zip.sha256"
    EXPECTED="$(cut -d' ' -f1 < "$TMP/update.zip.sha256")"
    ACTUAL="$(shasum -a 256 "$TMP/update.zip" | cut -d' ' -f1)"
...
ditto -x -k "$TMP/update.zip" "$TMP/unpacked"
APP="$TMP/unpacked/Birta Writer.app"
[ -x "$APP/Contents/MacOS/BirtaWriter" ] || { echo "the archive did not contain the app" >&2; exit 1; }
...
# (`BirtaWriterCore.SystemRequirements`), and a second reader written in shell
...
if command -v lipo >/dev/null 2>&1; then
    BUILT="$(lipo -archs "$APP/Contents/MacOS/BirtaWriter" 2>/dev/null || true)"
    case " $BUILT " in
        *" $MACHINE "*) : ;;
        *)
...
WAS_RUNNING=0
if pgrep -x BirtaWriter >/dev/null 2>&1; then
    WAS_RUNNING=1
    echo "→ asking the running Birta Writer to quit (it flushes its buffer first)"
    pkill -TERM -x BirtaWriter || true
    for _ in $(seq 1 100); do
        pgrep -x BirtaWriter >/dev/null 2>&1 || break
        sleep 0.1
    done
    if pgrep -x BirtaWriter >/dev/null 2>&1; then
        echo "update.sh: Birta Writer is still running after 10s. Quit it from the menu bar and re-run; nothing was replaced." >&2
        exit 1
    fi
fi
...
if ! mv "$STAGE" "$DEST"; then
    echo "update.sh: could not move the new app into place." >&2
    if [ -d "$OLD" ]; then
        mv "$OLD" "$DEST"
        echo "update.sh: the previous copy is back at $DEST." >&2
    fi
    exit 1
fi
rm -rf "$OLD"
