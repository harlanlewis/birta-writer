...
#     build running out of a checkout's `mac/build/` is in scope.
#   - `com.birtalabs.birta-writer.plist`. That is the app's REAL settings domain: a
...
for pid in $(pgrep -f "mac/build/Birta Writer[^/]*\.app/Contents/MacOS/" || true); do
...
shopt -s nullglob
for plist in "$HOME"/Library/Preferences/com.birtalabs.birta-writer.*.plist; do
    name="$(basename "$plist" .plist)"
    # Anything strictly under the prefix is a throwaway, and that is exact
    # rather than a heuristic: the app reads `com.birtalabs.birta-writer`, its own
    # bundle id, and the ONLY thing that ever names a different domain is
    # `BIRTA_MAC_DEFAULTS_SUITE` (`Preferences.swift`), which exists so a
    ...
    if [ "$name" = "com.birtalabs.birta-writer" ]; then continue; fi
...
shopt -s nullglob
LEFTOVERS=("$HOME"/Library/Preferences/com.birtalabs.birta-writer.*.plist)
shopt -u nullglob
...
case "$MODE" in
    ...
    check)
        if [ "$FOUND" -gt 0 ]; then
            echo "reap: $FOUND thing(s) left on the machine. Run: bash mac/scripts/reap.sh --reap" >&2
            exit 1
        fi
    ...
    report)
        if [ "$FOUND" -gt 0 ]; then
            echo "reap: $FOUND thing(s) to clear. Run: bash mac/scripts/reap.sh --reap"
        else
            echo "reap: nothing left behind."
        fi
        ;;
