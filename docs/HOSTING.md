...
4. `acquireVsCodeApi`, a global function defined before the module runs, returning an object with `postMessage`, `getState` and `setState`. The bundle calls it once. In a frame, `postMessage` hands the message to the parent window, and `getState` and `setState` hold the view-state bag (folds, scroll, table widths) for as long as the host wants them kept. The formatting row's open state is not in the bag: it is the host's, declared in the boot blob as `formattingRowExpanded` and reported back as the `formattingRowExpanded` message.
