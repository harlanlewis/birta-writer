...
The request-and-reply pairs every host takes part in, one in each direction; the pairs a capability adds (`projectFiles` and its directory listing, `stripTooltip`, the connectors) are listed beside the capability in `shared/hostProfile.ts` and typed in `shared/messages.ts`:
