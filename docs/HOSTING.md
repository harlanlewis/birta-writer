...
1. The palette. Link `dist/hostPalette.css` before `dist/webview.css`. The bundle's stylesheets use `--vscode-*` variables throughout, and inside VS Code the workbench injects them; any other host links this file instead. The theme is chosen by a class on `body`, `vscode-light` or `vscode-dark`. A host may redeclare any of those variables in a stylesheet placed after the palette's link, on a selector that beats or ties the palette's (`:root:has(body.vscode-light), :root:has(body.vscode-dark)` outranks the light block's `:root` and ties the dark block's, and a tie goes to the later declaration); that is how the Mac app applies a VS Code colour theme (`mac/Sources/BirtaWriterCore/VSCodeTheme.swift`), each workbench colour id becoming the variable of the same name. Code-block tokens are the one thing the palette does not colour by workbench id: `webview/components/codeBlock/codeBlock.css` reads a `--host-token-<class>` variable per Prism class, with its style and weight beside it, and falls back to the terminal palette when a host declares none.
...
4. `acquireVsCodeApi`, a global function defined before the module runs, returning an object with `postMessage`, `getState` and `setState`. The bundle calls it once. In a frame, `postMessage` hands the message to the parent window, and `getState` and `setState` hold the view-state bag (folds, scroll, table widths) for as long as the host wants them kept. The formatting row's open state is not in the bag: it is the host's setting, declared in the boot blob as `formattingRowExpanded`. A host that arranges `formattingInSecondRow` gets a switch for it in the page's gear menu, which posts `setFormattingRowExpanded` as a request rather than flipping anything; the host stores the answer and pushes it to every page, the asker included, as `setFormattingRowExpanded`. A host that does not arrange the row never receives the request and needs no handler.
...
The profile is settled before the page exists, and the page bakes decisions on it. One capability is not: `hostCapabilitiesChanged { capabilities }` replaces the whole list under a live page, for a host whose user can move one, and the Mac app sends it when `/ai` is switched on or off. `LIVE_HOST_CAPABILITIES` in `shared/hostProfile.ts` names which may travel that way, and carries the constraint on membership: a toolbar item gated on a capability is built once, at panel construction, so a capability arriving later finds no item to place.
...
The request-and-reply pairs every host takes part in, one in each direction; the pairs a capability adds (`projectFiles` and its directory listing, `folderIndex` and the index of a document's folder, `stripTooltip`, the connectors) are listed beside the capability in `shared/hostProfile.ts` and typed in `shared/messages.ts`. `folderIndex` is the one that subscribes: a single `requestFolderIndex` is answered at once and then again, unasked, whenever the folder changes, and the host resolves every reference in it the way it resolves a click, since the editor draws the edges it is sent and resolves none itself. The page sends that request only when its `folderGraph` setting is on (`window.__i18n.folderGraph`), whatever the host declared: the capability says the host can answer, the setting says the editor asks, and a host that wants the Backlinks and Graph tabs declares both.
...
| `requestPaletteCommands`, from the host | `paletteCommands { commands }`, from the editor | The editor's commands as the host's own palette can list them, gated the way the gear and the slash menu gate them. Answered on request, and once a host has asked, sent again unasked whenever the publishing targets change; a host that never asks is never told |
...
| `hostCapabilitiesChanged { capabilities }` | Replaces the capability list the page is running under, leaving the arrangements and the shortcuts as declared. For a host whose user can move a capability; see the profile section above for which ones may |
...
- Persisting across a reload. The view-state bag lives in the stub's `getState` and `setState`, and a stub that forgets loses folds, scroll and table widths on every load. The document itself is the host's from the first `update` on, and what a host owes it (saving, external edits, conflicts) is not this document's subject: it is [`PERSISTENCE.md`](PERSISTENCE.md).
