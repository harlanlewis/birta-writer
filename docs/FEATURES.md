...
- Frontmatter. YAML frontmatter renders as an editable key/value table at the top of the document, collapsible via `birta.frontmatterExpanded`. A block carrying Open Knowledge Format provenance gets a quiet label beside the collapse toggle, reading the status it declares, whether a person or only a machine countersigned it (`generated` and `verified`), and the day a `stale_after` deadline passed. It is read from the block and never written back, and it stays legible while the table is collapsed. A field the format does not define is left alone, as the format requires.
...
- Ask your agent from the caret. `/ai`, Space, then a request in plain words (`/ai add a mermaid diagram of the flow above`), then Enter. Birta composes one line, your request plus a `path.md#L12` reference to the caret, saves the document so that reference names what is on disk, and hands the line to the agent you already run per `birta.agent.command` (asked once, on first use): a shell command such as `claude -p {prompt} --permission-mode acceptEdits --output-format stream-json --verbose`, VS Code's Chat view, or the clipboard. Nothing is sent to a model by Birta itself, and it is one request each time, not a conversation. Ask Agent in the palette does the same and asks for the request in an input box.
...
- Backlinks and a graph of the folder (`birta.folderGraph`, off by default). With it on, the review sidebar gains a Backlinks tab listing every note in the workspace folder that links to this one, by a link, a `[[wikilink]]` or an Open Knowledge Format `sources` entry, and a Graph tab drawing the note with its neighbours, with the whole folder a click away. The folder is indexed when the sidebar is opened and a note re-read when it is saved; off, nothing in the folder is read. Birta Writer for Mac has both tabs on in a folder window.
...
| `birta.syntax.sets` | `["gfm", "obsidian", "pandoc", "notion", "calc"]` | The flavors of Markdown you write for, so the editor offers only the formatting they understand. Independent and overlapping; CommonMark is always available and is not in the list. Never changes what a document renders |
...
| `birta.folderGraph` | `false` | Backlinks and Graph tabs in the review sidebar: which notes in the workspace folder link here, and a graph of the notes around this one. Off, nothing in the folder is read |
| `birta.agent.command` | `""` | Where `/ai` and Ask Agent hand your request: a shell command with `{prompt}` (`claude -p {prompt} --permission-mode acceptEdits --output-format stream-json --verbose`), `chat` for the VS Code Chat view, or `clipboard`. Empty asks on first use. Never read from a workspace |
