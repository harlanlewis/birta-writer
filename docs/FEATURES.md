...
- Ask your agent from the caret. `/ai`, Space, then a request in plain words (`/ai add a mermaid diagram of the flow above`), then Enter. Birta composes one line, your request plus a `path.md#L12` reference to the caret, saves the document so that reference names what is on disk, and hands the line to the agent you already run per `birta.agent.command` (asked once, on first use): a shell command such as `claude -p {prompt} --permission-mode acceptEdits`, run in the background by default (`birta.agent.mode`), VS Code's Chat view, or the clipboard. In the background there is no terminal: a small dot pulses in the gutter beside the request while the agent works, clicking it cancels the run, and when the agent finishes its edit lands in the editor and undoes with Cmd+Z like a paste; if you kept typing meanwhile, the edit is merged around yours. Set the mode to `terminal` to watch the run in one reused Birta AI terminal instead. Nothing is sent to a model by Birta itself, and it is one request each time, not a conversation. Ask Agent in the palette does the same and asks for the request in an input box.
...
| `birta.agent.mode` | `"background"` | How a shell command in `birta.agent.command` runs: `background` (no terminal, a gutter dot while it runs, the edit undoes like a paste) or `terminal` (one reused Birta AI terminal) |
