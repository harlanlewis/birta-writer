...
Which record matters, and getting it wrong is how such a menu lies. The menu bar belongs to the application and its rows act on the front window, so what they draw has to be that window's answer. A setting persisted for the whole process is the wrong source the moment a second window exists: it holds whichever window answered last, so a row can draw one window's state over another's and then invert what it claimed when picked. Each window keeps its own record, seeded from the stored settings at every page boot and updated as its own page reports each flip; the stored settings stay the store, because they are what the next window opens with. The rule underneath is the one about lying: nothing outside the page may hold a value the page can change without telling it, so a switch that reaches a native surface has to be one the page reports as the reader flips it, and the surface has to read the record belonging to the window it will act on.
