...
| aider | 0.86.2, 2026-09-19 | `--model MODEL`, same line | documented, not read | none published |
...
Cline is the case that says why the value shapes matter more than the name list. Its `--thinking` was in `EFFORT_FLAGS` all along, so it was found and `supportsEffort` was true, with an empty scale behind it: the composer drew an effort button whose menu held only the default the user already had. Reading the flag's own value list is what turns that into a control. The name list is the weaker path for exactly this reason, and a flag found only by name should be read as a warning that its values were not understood.

Two harnesses are read incompletely, and both are recorded rather than papered over.

aider documents `--reasoning-effort` and publishes no values for it, so the shape rule cannot see it and `EFFORT_FLAGS` does not name it. Naming it would turn the picker on with nothing in it, which is the empty menu Cline was drawing: the effort menu, unlike the model menu, carries no free-text row, so that is the change to make first and the name second.
