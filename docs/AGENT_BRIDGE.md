...
### Skills: the third picker, and why it is not a flag

Beside the model and the effort the composer offers the agent skills on the machine: the `SKILL.md` folders under the workspace (`.claude/skills/`, `.agents/skills/`), under the home directory (the same two), and the one folder the configured harness keeps for itself (`HARNESS_SKILL_HOMES` in `src/agentBridge/agentSkills.ts`, keyed by the binary's name). A skill's `name` and `description` are the spec's two required frontmatter fields and are read off the file; a folder lacking either is not offered. The scan runs when the panel asks for capabilities, costs a few directory listings, and never waits on anything.

A skill is part of the request, not a flag. No harness on the survey documents a flag for one, and the composed line is the one thing every harness reads, so the pick is a prefix on that line (`composeAgentRequest` takes it) and `setTemplateFlag` is never involved: nothing can be spelled wrong, and the setting is not written. Two spellings, chosen by `skillPrefixFor` from the route: `/name In file.md#L12: request` for the harnesses that resolve a slash command as the skill itself (Claude Code, Cursor's agent), and `Use the name skill. In file.md#L12: request` for those that match skills by description, and for the Chat view, which reads a leading slash as one of its own commands. A name the harness does not know falls through to the plain request: probed on Claude Code 2.1.278, `/no-such-skill request` answers the request with a note that the command is not installed. That is a safer floor than the flag pickers have, where a wrong spelling is a command that fails.

The scan is exhaustive only for the paths it lists. A plugin's skills, an enterprise-managed set and an account-synced one (`~/.claude/skills/synced/` on one machine) are outside it, so the picker keeps a free-text row and the list is never rendered as "the skills", by the same rule that keeps `modelExamples` honest. The description is the user's own file, drawn as text and never as markup. The picker is gated on the `agentSkills` host capability, which the extension declares and the Mac app does not: it runs an agent and scans no folder, so it gets no picker rather than an empty one.
...
The reading half is `src/agentBridge/agentProgress.ts`, ported to Swift for Birta Writer for Mac as `mac/Sources/BirtaWriterCore/AgentProgress.swift` and held to it by one set of cases (`shared/__fixtures__/agentProgressCases.json`, read by both suites) plus `shared/__tests__/agentProgressPort.test.ts` for the constants and words a case list cannot see. It tells harnesses apart by the SHAPE of what they print rather than by name. Two structured shapes are recognized, each reduced from output captured from a real run, with the version and the date, the way the help fixtures are:
...
Birta does not add the flag that turns any of this on. Adding an argument to a command line somebody else wrote is how a request fails rather than differs, which is the rule the capability probe already keeps, and one of the two flags above has a prerequisite that no help text states (Claude Code's `--output-format stream-json` needs `--verbose`, which only its error says). What Birta does own is the templates it offers for itself, and those ask for events on both surfaces: the extension's two background picks (`CLAUDE_BACKGROUND_TEMPLATE` and `CODEX_BACKGROUND_TEMPLATE` in `src/agentBridge/askAgent.ts`) and the Mac app's Settings pull-down (`AgentPreset.template` in `mac/Sources/BirtaWriterCore/NoteModel.swift`). They are two lists because Swift cannot import the TypeScript, and they are held equal, command for command, by `shared/__tests__/agentBackgroundTemplates.test.ts`. A template that asks for none still runs exactly as it did, and its notice is the clock: that is every other preset in the Mac list, and any command somebody wrote themselves. The terminal templates ask for none on purpose: the user is reading that output, and events read worse than prose.

The guard is there because the two lists had already drifted, and each read as correct on its own. The Mac app shipped both commands without their structured flags, so a reader who took a preset and never edited it got the closing sentence after a silence where the extension showed the run working. Nothing else in the tree could see it: a preset is a string, and the list it should match is in another language.

Changing a template changes what a FRESH install runs and nothing else. `Prefs.agentCommand` falls back to the default preset only when the defaults domain holds no command, and both writers are gestures, so a command somebody already chose is never rewritten under them; the pull-down reads a stored command back by its program alone (`AgentPreset.matching`), so one chosen before a flag existed still names its tool rather than reading as Custom.

One place the Mac app then has to differ from what it prints: Settings' Test button runs the command once and shows what came back, and for a structured command what came back is events. The headline already says whether it worked, so the box under it is reduced by the same reader, through `AgentProgressReader.transcriptLines` (`AgentProgress.swift`), which is the one thing in that file that is not part of the port. It differs from the corner in three ways, each because a sheet is not a glance: every line is kept rather than the newest of a chunk, a narration is kept whole rather than cut to its opening and clamped, and a plain line survives a structured stream instead of being dropped. The last is the one that matters, since a run that emitted events and then failed says why in prose. Reduced, never replaced: a transcript with no events in it, and one whose events all said nothing, reach the box exactly as the child printed them.
