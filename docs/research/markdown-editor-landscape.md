...
1. **Security-first, local by default — no external services (the primary constraint).** The overriding design principle is that a document and its assets never leave the machine. Images are stored on local disk with MD5 dedup — there is **no image-upload path, no bundled cloud host, no telemetry, and no third-party service the editor depends on to function.** The AI handoff targets a *local* Claude Code terminal/extension, not a remote API. We deliberately **removed** the inherited server-upload capability (settings and all) rather than keep a dormant network path as a liability — keeping this posture airtight is a first-order goal, not an afterthought.
