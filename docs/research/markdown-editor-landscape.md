# Why Birta Writer exists — a survey of Markdown editing in VS Code

*Research compiled 2026-07-05 (feature references updated as the product shipped). This document justifies three decisions behind **Birta Writer**: (1) why a WYSIWYG Markdown editor for VS Code is worth building at all, (2) why we chose to fork [`git-xing/md-wysiwyg-editor`](https://github.com/git-xing/md-wysiwyg-editor) specifically, and (3) why we **hard** forked it rather than tracking it or contributing upstream.*

> **Scope.** This is the long-form research backing the fork. The canonical, maintained docs are the source of truth for what Birta *is today*: `README.md` (§ "Why this fork" — thesis and priorities), `docs/BENEFITS.md` (shipped capabilities and the compatibility table), and `docs/POSITIONING.md` (brand and naming). Where this survey and those docs disagree, they win.
...
3. **No incumbent combines the full feature set** we consider table stakes: **true ProseMirror WYSIWYG + in-place editing of the real `.md` with minimal diffs + first-class table/image UX + a strictly local, no-network-egress posture + active, English-first maintenance.** Each incumbent has some of these; none has all.
4. **Security and privacy are not a design priority for the mainstream.** The incumbents that do touch AI or images reach for cloud services. Birta takes the opposite stance as a first-order constraint: **local by default, no external services, no network egress.** A document and its assets never leave the machine — image storage is local disk with MD5 dedup, there is no bundled cloud host, no telemetry, and no remote endpoint the editor talks to. Where the fork base carried outbound paths (a server image-upload option, a "Send to Claude" handoff), those were **removed** rather than kept as dormant liabilities.
...
This is not a rescue fork. **Upstream is not abandoned** — it shows recent activity (e.g. v0.2.2 in June 2026). We make no claim about how responsive it is to outside contributors; we haven't assessed that, and it isn't the point. The split is about **direction**: a deliberate choice to build to our own strong opinions about what a Markdown editor should — and shouldn't — do, under a design philosophy upstream doesn't share.
...
1. **Security-first, local by default — no external services (the primary constraint).** The overriding design principle is that a document and its assets never leave the machine. Images are stored on local disk with MD5 dedup — there is **no image-upload path, no bundled cloud host, no telemetry, and no third-party service the editor depends on to function.** We deliberately **removed** every outbound path the fork base carried — the server image-upload capability (settings and all) and the "Send to Claude" handoff — rather than keep dormant network paths as liabilities. Keeping this no-egress posture airtight is a first-order goal, not an afterthought.
...
- **Local-first and private by design (the guiding principle).** No bundled cloud services, no telemetry, no external dependency to function, no network egress. Images are stored on local disk with MD5 dedup with **no upload path at all** (the inherited server-upload feature was removed), and the inherited "Send to Claude" handoff was removed too. This is the axis on which the project most sharply diverges from upstream's broader, cloud-embracing scope (see §4).
...
- **First-class table & image UX.** Row/column drag-handles and insert-lines; image paste / drag-drop / picker with local MD5-dedup storage — always written to disk, never uploaded.
