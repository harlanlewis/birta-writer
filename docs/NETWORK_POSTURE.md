...
- **It ships off, and Jot's first run offers it on.** The stored default is `false`, exactly as `birta.network.enabled` is, so an install that never reaches the first-run screen makes no request. That screen shows the switch, labelled "Rich link previews and embeds", already on, and writes that answer before it draws (`Prefs.applyOnboardingDefaults`), so what it displays and what is stored are the same thing from the first frame. The distinction it turns on is between a default nobody saw and an answer somebody was shown and can refuse in the same gesture: rung 1 is never entered for a person who was not offered the switch, and the offer is one click from a no. `applyOnboardingDefaults` writes only keys nobody has set, so a person who has already turned it off is never overruled by re-showing the screen.
