...
|---|---|---|---|
| 0. Nothing | No outbound request at all | Shipped, and the default. `birta.network.enabled` ships `false`, and Jot's `networkEnabled` ships `false`; with it off neither makes an outbound request. Jot's first-run screen is the one place the default is offered on, to a first-launch install only, and §2's Jot subsection has the argument | Everything, out of the box |
...
- **It ships off, and a FIRST-LAUNCH install is offered it on.** The stored default is `false`, exactly as `birta.network.enabled` is. `Prefs.applyOnboardingDefaults` writes `true` before the first-run screen draws, so the switch it shows and the value stored are the same thing from the first frame, and it runs only when `Prefs.isFirstLaunch`: an install with any setting stored, which is every install that predates that screen, sees the screen showing what it already does and has nothing written for it. So the value is on only for somebody who was shown the switch in the same moment, one click from a no, and rung 1 is never entered for anybody else. `reset` deliberately leaves `hasSeenWelcome` set, so a reset cannot turn it back on at the next launch.
