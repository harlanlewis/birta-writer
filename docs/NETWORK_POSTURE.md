...
|---|---|---|---|
| 0. Nothing | No outbound request at all | Shipped, and the default. `birta.network.enabled` ships `false`, and the Mac app's `networkEnabled` ships `false`; with it off neither makes an outbound request, and nothing but a person switches either on | Everything, out of the box |
...
| 2. A URL and your credential | The URL and a per-provider token, to that provider's pinned hosts | Shipped for GitHub, Linear and Asana; every other provider directed, not built | GitHub repository, issue and pull-request cards; Linear issue cards; Asana task cards; Jira, Confluence and Figma still to come |
...
Birta Writer for Mac makes rung-1 requests too, and the rung is what matters rather than the process making them: a URL the user typed goes to its own host and to where that host redirects, and nowhere else. The Mac app has no connectors, so nothing about rung 2 exists there at all. Rung 3 needs the paragraph below rather than a clause, because the Mac app can now keep its note in iCloud Drive.

Four differences from the extension. Three narrow what the extension does; the fourth is where the Mac app asks rather than defaults.

- **One switch, not four.** The Mac app has a single network preference, and with it off the app makes no outbound request of any kind. Embeds, link cards and paste-unfurl all ride it. There is no per-provider table because there are no provider cards to gate.

- **One switch, off, and nothing turns it on but a person.** The stored default is `false`, exactly as `birta.network.enabled` is, and no code path anywhere sets it to true: the first-run screen does not ask about the network at all, so there is no default-on to argue about and no install, new or old, reaches rung 1 without somebody moving that switch themselves. `Prefs.applyOnboardingDefaults` is where such a write would go, and its doc says why it must never grow one; `mac/scripts/measure.sh`'s onboarding arm asserts the absence on both a first launch and an existing install.
- **The guards are a second implementation, and are held to the first.** `mac/Sources/BirtaWriterCore/UrlGuard.swift` mirrors `src/utils/urlGuard.ts`, and `PageMetadataFetcher` mirrors the redirect, byte and time bounds around it. Neither language can import the other, so the cases live in `shared/__fixtures__/urlGuardCases.json` and both test suites read them: a rule enforced on one surface and not the other fails a test rather than becoming an exposure nobody compared. Add a case there, never in one suite. The content-security-policy is the third such pair, and it is the one that drifted before anybody compared it: the Mac app widened `img`, `frame`, `connect` and `media` to the whole of `https:` on the network opt-in, where the extension pins the provider hosts and grants connect and media nothing. It now restates those hosts as `EmbedHosts` in `mac/Sources/BirtaWriterCore/EmbedHosts.swift`, held to `EMBED_CSP_FRAME_HOSTS` and `EMBED_CSP_IMG_HOSTS` by `shared/__tests__/macCsp.test.ts`, and widens the same two directives the extension does and no others.
...
### The Mac app browsing Open VSX for a theme: asked for, to one host, carrying the words typed

The Appearance pane's Browse Open VSX… searches the Eclipse Foundation's registry (`open-vsx.org`) for theme extensions and, on Add, fetches the chosen one's VSIX and hands it to the same import a file from disk takes. Two requests, each made when a person opens the sheet (its first page, the category's most downloaded, with no search words), presses Return in the search field, or presses Add on a result, carrying the search words and the extension's name and nothing else: no document, no identifier, no setting. The search goes to the registry's host; the package goes to that host and to where it redirects the file, which today is the registry's own file host (`openvsx.eclipsecontent.org`), the same shape as the updater's archive following a release page. Nothing runs on a timer, and nothing rides the network switch, for the reason Check for Updates does not: pressing the button is the asking. `OpenVsx.swift` composes the search URL and reads the answer, and refuses a download URL that is not on the registry's own host, so the first hop never goes where a listing did not point; the redirect is the registry's to choose, and the one rule held over it is that it stays on https (`ThemeBrowser.RedirectGuard`). Open VSX rather than the Visual Studio Marketplace because that marketplace's terms admit only Microsoft's own products, where Open VSX has a public API; the popular themes are published to both, as this extension is.

### The Mac app updating itself: a rung of its own, on by default, and unattended

Birta Writer for Mac is on no app store and cannot be, so the only way to get a fix has been to notice a release happened and run a shell script, which is a thing nobody does. `Prefs.autoUpdate` ships on: at launch, and every couple of hours while the app is running, it asks `api.github.com` what the newest release of this project is, and if it is newer than the running build, says so. Check for Updates, on either menu or in Settings or in the About window, makes that same request whatever the setting says, because pressing it is the asking, and it does not turn the setting on. That is the request a person makes rather than a timer, and it is not the only one: Install on Next Launch fetches the archive when it is pressed, on the same reasoning. The check repeats rather than running once per launch because the Mac app is a menu-bar app people leave running for weeks, so a launch-only check stops happening for exactly the people who use it most; the interval is `UpdatePolicy.recheckInterval`, and the timer does not raise a version you have declined until a newer one exists, so a shorter interval buys a fix that arrives sooner and not a second prompt. Pressing Check for Updates clears that decline, which is deliberate and is the only way back to an offer you said no to. With the setting on, the download follows the check on its own: the archive is fetched, verified against its published checksum and unpacked into the temporary directory, and nothing is replaced yet. That is a change from what shipped before, where nothing was fetched until somebody confirmed a sheet, and it is the whole of what the extra bytes buy: by the time anybody is asked, or the machine goes quiet, there is nothing left to wait for.

Not over a connection somebody pays for by the byte. The automatic download is the one thing in this app that spends money without being asked, and a release lands most nights, so a Mac tethered to a phone would pay for one after another for something nobody requested. `NWPathMonitor`'s `isExpensive` (cellular or a hotspot) and `isConstrained` (Low Data Mode) both hold it off, and the check itself still runs, so the offer is still raised. The offer's own Restart button is deliberately not held to the same rule: somebody who presses it has asked, and Low Data Mode is a preference about background traffic rather than a refusal to ever update.

The rule the swap itself is built around is unchanged, and it is narrower than "never automatic" was: swapping the app somebody is TYPING INTO is not a thing to do behind them. That is a rule about the person at the keyboard, so a swap goes in on its own only where `UpdatePolicy.mayInstallUnattended` says every one of these holds: an update already staged, the setting still on, this version not one you declined, no offer on screen, no `/ai` run still going, and `UpdatePolicy.isUnattended` satisfied about the person, which is no window of the app on screen, nothing unwritten in any buffer, and a machine that has seen no input at all for `UpdatePolicy.unattendedIdle`. That last fact is read from the window server rather than from the app, because a menu-bar scratchpad is hidden nearly all the time and its own idea of idleness would call almost every session unattended. Every arm refuses on doubt, an unreadable idle time included, and a refusal costs nothing: the update stays staged and the next poll asks again a minute later. Where somebody IS there, the sheet still asks, and it now restarts into bytes that have already arrived.

Two things follow that a reader should be able to check. An unattended swap reopens the app without activating it (`open -g`), so a person who walks away and comes back finds the front application they left. And it is announced: the panel carries a notice naming the version that went in, which stays until it is dismissed rather than fading, because the panel is hidden most of the time and a message that faded would be spent on an empty screen. Declining a version suppresses both ways in and discards what was staged, so a no is an answer about that version and not only about the sheet.
...
Paste-unfurl in the Mac app writes on the same terms as in the extension: the fetched title arrives as an offer, and auto-apply is left at its default of false, so nothing reaches the file until the user accepts it.

`shared/embedProviders.ts` enumerates the hosts an embed card can reach, and the same table generates the content-security-policy grants on both surfaces: directly in `src/webviewHtml.ts`, and through the Swift restatement in `EmbedHosts` for the Mac app.

### Rung 0 includes diagram rendering, and that is not free by default

Mermaid, KaTeX, PlantUML, Graphviz and an SVG fence all render on this machine, with no request of any kind. For PlantUML that is a property worth stating explicitly, because the ordinary way to render it is the opposite: upstream PlantUML resolves `!theme <name>` and `!include <url>` over HTTP, and most editor integrations post the diagram source to a PlantUML server. Either would put *document content* on the wire, which is rung 3 (the rung that is not decided and not designed) rather than rung 1, no matter how routine it looks.
...
An SVG fence reaches that invariant a different way, and it is the one construct where the CSP is not the whole answer. There is no engine to compile the fetching out of: the fence holds markup the document author wrote, and `<image href>`, `<feImage href>` and a `url()` inside a paint or `style` attribute are all ways for that markup to name something remote. In the editor the CSP refuses every one of them. The exported HTML file has no CSP, so the reference is stripped at sanitize time instead (`webview/utils/sanitizeLoader.ts`); `<use>` and `<foreignObject>`, which can pull in a whole external document, are not stripped but dropped, because they are outside the sanitizer's SVG profile to begin with. What survives a reference is a `#fragment` or an embedded `data:` image, neither of which leaves the machine, and a link, because following it is the reader's own click. Pinned in both places: `e2e/svgRender` asserts the page issued no request beyond its own bundle, and `e2e/htmlExport` opens the exported file in a browser with nothing stopping it and asserts the same.

Inline HTML in a markdown body reaches that same sanitizer, and the strip is deliberately not scoped to the fence. `<img src="https://…">` and `style="background: url(https://…)"` are the same reference by another door, refused by the CSP in the editor and live in an exported file, and the style form is the one worth naming because it renders as nothing a reader would look at. Both are stripped now. The strip is surgical rather than blanket: a `style` attribute loses only the declaration naming the remote reference, so an author's CSS beside it survives, which is the same trade the escaping-property filter already makes. A remote `<a href>` is kept, in inline HTML exactly as in a fence. An ordinary markdown image needed no rule, and that asymmetry is what made inline HTML the outlier rather than this a new policy: a `![](https://…)` never reaches the exported file at all.
...
Pasted personal access tokens are the third rung, and Asana is the provider behind them. Asana's OAuth is a confidential-client flow, and a client secret shipped inside a distributed extension is not a secret, so a personal access token is the whole of what Asana offers a program running on the user's machine: the rung is the provider's choice rather than ours. Birta opens Asana's own instructions, takes the token in a masked box, checks it with one call before recording anything, and keeps it in SecretStorage. The box states what the token covers, because a personal access token carries whatever the account can do and Asana issues no read-only kind; saying "read-only" without that would describe Birta's code rather than the grant. `app.asana.com` is the one host the credential may reach, and Asana's API answers nothing without it, so an unconnected card asks Asana nothing at all rather than trading the document's task ids for a 401.

Jira, Confluence and Figma are still to come, and Atlassian is the case invariant 8 was written for: it waits, and it waits on Atlassian rather than on us. Both rungs it could arrive on are closed, for different reasons, and neither is a design we are missing.

Its OAuth would pin perfectly. Under 3LO the credential-bearing hosts are `auth.atlassian.com` and `api.atlassian.com`, with the tenant in the path rather than in the host, so the exact-host rule would hold with nothing added to this seam. What closes it is client authentication: Atlassian's authorization server advertises only `client_secret_basic` and `client_secret_post`, with RFC 8414's `none` absent, so there is no public client and the token exchange needs a secret that a distributed extension cannot hold. PKCE exists there as a challenge method and not as a substitute for that secret. The request for one is open and unimplemented on Atlassian's own tracker as ECO-283.

Its API token is user-supplied and so would be shippable, but it is sent to `<site>.atlassian.net`, a host per customer, which is exactly what the exact-host rule cannot express. Widening that rule to a suffix match would hand a credential to any tenant a document happened to name, which is the shape invariant 6 forbids. It would also need a per-spec credential encoding, since Atlassian authenticates a token with HTTP Basic rather than a bearer.

So the thing to watch is ECO-283. If Atlassian ships a public client, Atlassian becomes an ordinary `oauth-pkce` row against two fixed hosts and needs no new mechanism at all, which is why no work has been done on admitting a user-supplied host into the allowlist.
...
That is the live question, and it is a column the persistence contract ([`PERSISTENCE.md`](PERSISTENCE.md)) names and does not yet hold: credentials are a host's to provide, and no ticket carries them for a host that is not VS Code.

---

## Tracking

MAR-198 (connector foundation: the seam and GitHub shipped, the rest of the roster open), MAR-186 (provider roadmap), MAR-179 and MAR-199 (the shipped consent ladder and its application scope, both Done), MAR-232 (rung 3, gated). The open portability question (a keychain, a callback route and consent scope off VS Code) has no ticket; [`PERSISTENCE.md`](PERSISTENCE.md) names it as the credentials column it does not yet hold, and MAR-225's capability taxonomy records it as its known gap.
