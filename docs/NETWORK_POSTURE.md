...
Jira, Confluence and Figma are still to come. Atlassian in particular is blocked rather than merely unbuilt: Jira and Confluence live at `<site>.atlassian.net`, a host per customer, which the exact-host rule above does not admit, and widening it to a suffix match would hand a credential to any tenant a document happened to name. Whether a user-supplied site host may enter the allowlist at connect time is an open decision, and it may not need making at all: Atlassian's OAuth 3LO reaches both products at `api.atlassian.com`, one fixed host with the tenant in the path, which would put Atlassian on the second rung where the problem does not arise.
