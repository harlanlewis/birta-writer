...
One gap in the app's rule is deliberate and worth stating, because nothing about it is visible from the outside. While an `/ai` run is in flight the check stands down entirely, on the ground that the run is editing the bound file at this window's request and a finished run reconciles through `AgentLandingPolicy`, which keeps the agent's version beside the note rather than losing it. Autosave goes on writing during that window, so a THIRD program editing the same file while an agent run is working is the one case the app can still write over. The alternative, asking about the file the user just asked an agent to rewrite, is worse for the case that actually happens.
