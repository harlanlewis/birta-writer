...
- It helps me write better:
    ...
    - Customizable draft-management tools for `[TK]` notes, `TODOs`, and inline comments. Each marker is highlighted where it sits, so unresolved bits are visible while writing, and the review sidebar lists them all for jumping between. The in-text highlighting is a switch under Proofreading in the toolbar's settings menu and on the sidebar's Notes tab, independent of the proofreading switches, because your own notes are content, not findings.
...
- Markdown files open automatically in Birta Writer. Crosslink them. Click to navigate across pages. Birta Writer for Mac also shows what links back: a Backlinks list and a graph of each note's neighbours, or of the whole folder with its most connected notes, the notes nothing links to, and the links that name no note. In VS Code the same tabs are there behind `birta.folderGraph`, off by default, because an extension built for a vault does that job better. View files side by side in a window.
- Open GitHub Copilot, Claude Code, or another AI assistant in a VS Code sidebar to chat with and modify your documents live, and let it see what you're looking at. VS Code hides a custom editor from the active-editor API agents read, so Birta bridges the gap. Copy a precise `file.md#L12-L20` reference (or the reference plus the selected lines, quoted as real markdown) to paste into any agent, ask from the caret with `/ai` followed by a request (Birta composes the request and the reference into one line, saves, and hands it to the agent command you chose, in the background with a stop pill in the gutter while it runs and an edit that undoes like a paste, or in one reused terminal, or the Chat view or clipboard; one request at a time, no model of its own: which model answers is a flag on the command you chose, so Birta can run a smaller and faster one than your interactive sessions do, and the slash menu names it before you send. `/ai-advanced` opens a composer instead, for a request that needs more than a line: attachments by paste, drop or button, and a model and effort for that request alone. What those controls offer is read from your own agent's `--help`, so no list of models ships with the editor and none goes stale; a tool that documents no such flag simply gets no such control), or let a tool-using agent pull your current file, caret, and selection directly: a Language Model Tool for Copilot agent mode, and a public API for any extension. No more hand-directing the agent to the lines you mean.
...
A list you start by typing has no file to read the choice from, so the choice is the marker you typed: type `* ` and the file gets `*`, type `1) ` and it gets `1)`. It is the same principle one step earlier: the spelling is yours either way, whether you wrote it in the file or in the editor. A marker typed directly below an existing list continues that list when it is the same character, and starts a second list when it is not, which is what those same two lines mean in a file.
...
Editing alongside tools that also write your files is normal now. Birta surfaces the collision and lets _you_ pick the winner, instead of guessing a merge or quietly discarding a side. The editor never writes your document on its own, and never reverts one holding unsaved work: the single revert it makes unprompted is on a document already identical to its file, to clear an unsaved-changes flag with nothing left to describe, and it stands down while the file on disk has moved. It never takes over a raw editor holding unsaved changes: rendering one means closing it, and closing a tab with unsaved work forces a decision you did not ask for, so Birta leaves that tab alone until you've saved it yourself. Every mode switch closes the tab it is leaving before it opens the one it is going to, so a switch never leaves the same file open twice.
...
A card contacts only the named provider of a bare link on its own line: a YouTube thumbnail at render, and a title lookup at that provider's own oEmbed endpoint, so the card can name what it embeds. The player is created only when you click, whether that's YouTube in privacy mode (`youtube-nocookie.com`), a Loom or Vimeo video (Vimeo's always loads with its `dnt=1` do-not-track flag), a live Figma frame, a published Google document through Google's own embed endpoints, a Miro board's login-free live view, or a CodePen, CodeSandbox or StackBlitz playground in that provider's own embedded editor. Each provider's exact hosts are pinned in one shared table that also generates the content-security-policy the editor page carries, on every surface it runs on. Never a wildcard, never an aggregation service.
...
Other services connect the way each one allows, and the difference is worth knowing because it decides who holds the key. Linear sends you to its own consent page in your browser and hands back a token scoped to reading; Asana offers a program on your own machine nothing but a personal access token, so Birta opens Asana's instructions, takes the token in a masked box, and checks it with one call before it records anything. Whichever way it arrives, the credential goes into your keychain rather than a settings file, it never reaches the webview, it is never carried to another machine by settings sync, and it is sent only to the one host that service's cards are read from. A token you paste is the case that deserves the most care from us, because Asana issues no read-only kind and the token therefore carries everything your own account can do: the box says so before you paste, rather than letting "read-only" describe our code instead of your grant. Until you connect, an Asana card is built from the link alone and asks Asana nothing, because Asana's API answers nothing without a credential and a request made anyway would spend your task ids on a refusal.
...
| ------------------------- | -------------------------------------- | --------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------- |
| **Obsidian**              | Yes, vault of `.md`                      | Yes, directly                             | Wikilinks, `==highlights==`, `> [!callouts]`, footnotes, math, and YAML frontmatter render or round-trip; `#tags`, `^block-ids`, `![[embeds]]`, `%%comments%%` stay as preserved text                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | Strong                                 |
| **Foam**                  | Yes, `.md` (VS Code-native)              | Yes, directly                             | Same wikilink family as Obsidian; its optional CommonMark link-reference-definition shim is preserved, not inlined away                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Strong                                 |
| **"Second Brain" / PARA** | Yes (a folder convention, not a format) | Yes, directly                             | Nothing tool-specific to preserve: it's just folders of Markdown                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Strong                                 |
| **Logseq**                | Yes, `.md`                               | Yes, opens (round-trip tested)            | Logseq is an outliner: every block is a bullet and tab indentation encodes the block tree, so a file renders as one big nested list. Untouched lines (tabs, `key:: value` properties, `((block-refs))`, `TODO`/`DOING`/`[#A]` markers, `CLOCK:` timestamps) are byte-preserved through an edit elsewhere, and an edited line keeps its org tokens unescaped and its own tab indentation, so the blocks nested under it stay nested (pinned by a round-trip test suite that types into every block of each fixture and re-parses the result). Block _moves_ within a tab-indented outline keep the file's own indentation too, so a dragged or Alt+moved block lands at the depth you left it at. That holds whatever the block contains: plain prose, a heading in the outliner's own `- # Title` shape, a quote, a fence, or a table, whose rows re-base to the depth the bullet lands at rather than keeping the indent of the nesting they left. A bullet whose content is a horizontal rule keeps its nesting through a save and a reopen, and through a Tab indent. An outline that spells one level two ways (a tab in one place, spaces in another, which is the ordinary state of a file more than one tool has edited) moves correctly as well: the moved bullet is re-spelled into the convention its neighbours use instead of reopening one level deeper. The whole-file move gate runs against the Logseq fixtures with no carve-out. One shape is still open and worth knowing if you hit it: a moved item whose marker differs from its saved counterpart _only_ in indentation comes back in the editor's spelling rather than the file's (its content survives intact) | Text-edit safe; structure renders flat |
| **Quarto** (`.qmd`)       | Yes, single `.qmd` file                  | Caution, needs a file association (see below) | Pandoc Markdown doesn't subtract from CommonMark, so untouched content round-trips safely; but executable ` ```{r} ` cells, `::: {.callout}` fenced divs, shortcodes, cross-refs, and citations are preserved as inert text/code, not understood                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Safe, not fluent                       |
| **MDX** (`.mdx`)          | Yes, `.mdx` file                         | Yes, directly (MDX mode)                      | `.mdx` opens in its own MDX mode: prose edits work as in Markdown, while JSX elements, `{expressions}` and `import`/`export` render as inert, never-executed blocks and round-trip byte-perfect. A JSX block's plain string attributes (`title="..."`) show as a small form and can be edited in place, written back into the file's own bytes with their original quoting; expression, spread and boolean attributes and the element's children stay read-only. Markdown nested inside a JSX element is shown but not yet editable. A file that is not valid MDX (a stray `{`, an unclosed tag) cannot open visually: it falls back to the text editor with the parser's error, because MDX parse errors are fatal where Markdown has none                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | Text-edit safe                                  |
| **Roam Research**         | No, proprietary DB (JSON/EDN)           | No, only after Markdown export           | Moot until exported                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Not file-based                         |
| **Bear**                  | No, proprietary SQLite                  | No, only after export                    | Moot until exported                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Not file-based                         |
| **Emacs Org mode**        | Yes `.org`, but not Markdown            | No, a different language     | `* headlines`, `:PROPERTIES:` drawers, `#+BEGIN_` blocks aren't Markdown; a Markdown parser would misread them                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Wrong format                           |
...
- Logseq also opens, but it's an outliner: its whole-file structure, not just its text, rides on exact bullet indentation. Text edits are round-trip tested. Untouched lines keep their tab bytes and org tokens exactly, and the edited line keeps both its tokens unescaped and its own tab indentation, so its nested children stay children. Moving blocks, by drag or by keyboard, keeps the outline's own indentation as well, so a moved block re-opens at the depth you left it at, carrying its continuation lines and any nested code fence intact. That covers a block whose content is not a plain paragraph: a heading (the Logseq shape `- # Project Atlas`), a quote, a fence, a _table_. Editing inside such a block is held to the same rule as moving it. A bullet whose content is a _horizontal rule_ keeps its sublist through a save and a reopen. The compatibility row above has the detail, including the one shape still open.

- Markdown supersets are plain files, but they extend or alter the language, and the two rows differ now. `.mdx` opens in Birta directly, in an MDX mode that parses the real grammar; do not rename an `.mdx` file to `.md`, which routes it through the plain-Markdown parser where MDX's rule changes make an edit corrupting. Quarto has no mode of its own: Birta registers only `.md`, `.markdown` and `.mdx`, so a `.qmd` needs a rename to `.md` or a `workbench.editorAssociations` entry (e.g. `"*.qmd": "birta.editor"`), and is then safe to round-trip, its extensions surviving as inert text.
...
> A note on confidence: the claims above are machine-verified. One command, `pnpm fidelity`, drives a per-tool fixture corpus (authored from each tool's own documentation) through the production save pipeline and asserts the table's claims. The Strong, Text-edit safe and Safe rows round-trip byte-identically and keep every tool-specific construct named above through an edit; frontmatter, handled before the editor ever sees content, is verified by the extension-side suites. The MDX row's mode claims are asserted by `webview/__tests__/roundTripCorpusMdx.test.ts` and `mdxFormat.test.ts` over real MDX fixtures, islands and fatal-parse cases included. Two expected-corruption cases remain encoded as such: Org, and an `.mdx` file renamed to `.md` so it bypasses MDX mode. For both, an untouched save is still byte-identical, but an edit corrupts the edited construct. For Org that is the edited headline itself, which Birta reads as a bullet, while its property drawer and the rest of the file, keyword lines included, are left alone. The fixtures and how to read the suites live in `webview/__tests__/fixtures/tools/README.md`; CI runs the same assertions on every PR and push to main, so a claim that stops being true fails the build.
