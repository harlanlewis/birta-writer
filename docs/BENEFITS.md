...
The lock is structural for everything the editor itself owns: every change that goes through the editor's own transaction boundary is refused there, so a feature built on that boundary is covered whether or not anyone remembered this mode, and the affordances follow: a control that cannot act is dimmed rather than left to look live and do nothing. The few surfaces that write around that boundary (the metadata panel, a fullscreen code editor) each ask the mode themselves. Editing raw Markdown still opens the file for editing, because that intent is explicit.
