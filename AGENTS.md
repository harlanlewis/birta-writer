...
`.claude/prose-guard` opts this repository into the mechanical layer, and it judges EVERY line of prose rather than only list items. Scope is by convention so a new document is covered the day it lands: `CHANGELOG.md`, `README.md`, `AGENTS.md`, `CLAUDE.md`, and anything under `docs/` or `.claude/`. Either that file or the older `.claude/changelog-guard` switches it on, and the guard runs while either is present, so retiring it means removing both. Verify which one is live before deleting one: a deny names the marker it actually read, and a marker the installed plugin does not look for is a guard that is off with nothing to say so.
...
- An enumeration must assert its own size, because a sweep that reached nothing passes. A schema-wide gate over every block type reported zero types enumerated and went green: it built each node from one editor's schema and inserted it into another's, and every `makeCorpusEditor` constructs its own `Schema`, so the types mismatched, `tr.insert` produced no step, and each type fell through a skip. Build fixtures from the editor under test, and assert both the covered count and the list of what could not be reached (`blockAddDeleteResidue.test.ts` is the worked example). This is the same failure as sampling too narrowly, one step further along: the assertion is over an empty list either way.
...
- A mutation run needs an arm proving each edit landed. A mutation that silently failed to apply reads as a survivor, so the run reports coverage over mutations it never made. This is the empty-oracle trap one step earlier.
- When the subject is a predicate, check what the assertion discriminates. One that holds at the threshold, at the variant it claims to rule out, and at a hardcoded `if (true)` is decoration, and a differential oracle fed the same input twice agrees with itself whatever the predicate says.
