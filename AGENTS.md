...
The class of defect this exists for: WebKit will not hold an insertion point at a DOM position flanked by `contenteditable=false` widgets, and re-anchors to the end of the previous block, so the next character typed lands on the previous line. Chromium tolerates the same arrangement. Every widget decoration at a block's first inline position therefore takes a negative `side`, so it sorts before the caret rather than around it; `plugins/emptyLineHint.ts` and `plugins/headingFold/foldDecorations.ts` are the two that sit there, and `e2e/enterCaret` pins the gesture in both engines.
