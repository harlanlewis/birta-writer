...
`.claude/prose-guard` opts this repository into the mechanical layer, and it judges EVERY line of prose rather than only list items. Scope is by convention so a new document is covered the day it lands: `CHANGELOG.md`, `README.md`, `AGENTS.md`, `CLAUDE.md`, and anything under `docs/` or `.claude/`. Three of its four rules never mention the CHANGELOG, so the marker is named for what it judges.

Both markers are present while the rename lands. The plugin reads `.claude/changelog-guard`, so that file is the live switch and `rm` on it retires the guard everywhere; delete it once the plugin reads `prose-guard`, and not before, because a marker the plugin does not look for is a guard that is off with nothing to say so.
