...
`.claude/changelog-guard` opts this repository into the mechanical layer, and it judges EVERY line of prose rather than only list items. Scope is by convention so a new document is covered the day it lands: `CHANGELOG.md`, `README.md`, `AGENTS.md`, `CLAUDE.md`, and anything under `docs/` or `.claude/`.
