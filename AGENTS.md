...
- Open is bound by the model, so the open path shows something before the model exists. `webview/firstFrame.ts` paints the first screen read-only from a prefix the block segmenter (`utils/blockSegmenter.ts`) has proven safe to cut, keeps what is typed meanwhile, and swaps to the live editor in the same task its DOM lands. The segmenter is the one answer to "where may this text be cut" and must stay text-in, cuts-out, never a parser; its corpus oracle is the parser itself.
