...
- Local install: `pnpm run install:local` (`scripts/install-local.mjs`) runs the whole handoff below in one command, for both surfaces: the extension into VS Code, and the Mac app into `/Applications` as `Birta Writer [DEV].app`. It never touches your `settings.json`, and the only manual step left is the window reload. Jot needs no reload; a running copy of that build is asked to quit, replaced, and relaunched. The DEVELOPMENT flavour, beside the release rather than over it: `Birta Writer [DEV].app` keeps its own settings, its own note and its own summon hotkey, so a change can be reviewed without taking away the copy somebody keeps their notes in. `BirtaJotCore.AppFlavor` holds what has to stay separate and why. The release copy is deliberately never touched by the handoff; it updates itself, or `jot/scripts/update-jot.sh` installs it by hand.
...
The surfaces are told apart only where prose has to tell them apart: Birta Writer for Mac, Birta Writer for VS Code. That is a qualifier, not a second product name, and it never reaches a display string. Its one load-bearing use is `jot/CHANGELOG.md`, where the misfiling guard needs a subject it can match.

`Jot` is retired from user-facing text and kept as the internal codename, so the `jot/` directory, the `BirtaJot` and `BirtaJotCore` modules, `com.birtalabs.jot`, the `BIRTA_JOT_*` variables and the `e2e/jotHost` suite all keep it. It is also reserved for a future quick-entry surface, which is why `JOT_PRODUCT_NAME` stays a separate constant from `PRODUCT_NAME` in `shared/product.ts` even though the two now hold the same string: they name a program and a product line, and only their spellings have converged.

A filename is not branding. The dated note template is still `Jot %Y-%m-%d.md`, because that word is a short stem in front of a date rather than a claim about what made the file.

Two identifiers must not follow a rename, and both fail quietly if they do. `com.birtalabs.jot` is the defaults domain, the flavour discriminator `AppFlavor` reads, and the prefix `jot/scripts/reap.sh` scopes itself by; changing it resets every stored setting. `ReleaseFeed.assetPrefix` is how an installed copy recognizes its own update in a release's assets, and a mismatch there returns nil rather than failing, so fixes ship and nobody receives them.
...
jot/                                          Birta Writer for Mac, the menu-bar app (SwiftPM) around dist/webview.js; jot/README.md
