...
mac/Sources/BirtaWriter/TitlebarActions.swift    The titlebar's buttons, in two rows around the name: the file explorer's toggle before it, New Note, Open (a menu over the recent files) and the Command Palette after it; which SF Symbol each takes, why the near alternatives are wrong, why room is held for a button that is merely not drawn and none for one the window cannot offer
mac/Sources/BirtaWriter/PaneGlyph.swift           The sidebar toggle's mark, a port of the page's own pane icon: why a stroke straddles its path, and the two guards (numbers in paneGlyphParity.test.ts, pixels in TitlebarSymbolsTests) that hold the port
...
pnpm test:coverage     # run tests + coverage report (coverage/); leaves out the corpus sweeps (CORPUS_SWEEPS in vitest.config.ts)
...
The Playwright browser cache is shared the same way and fails in the opposite direction, silently and a day later. Every repo on the machine draws its browsers from one directory outside all of them (`~/Library/Caches/ms-playwright` on macOS), and `npx playwright install` garbage-collects every build the installing repo does not pin. So an install run here to fix this suite can leave another repo's suite with no browser the next morning, and an install run there can do it to this one. Pass `PLAYWRIGHT_SKIP_BROWSER_GC=1` on any `playwright install`, which keeps the other builds and costs only disk. The tell that more than one repo is sharing the directory is more than one build of the same browser in it, and that is the normal state rather than a mess to tidy. There is no spare to delete: two repos that resolve different Playwright minors want different browser builds, so each set is the live one for a repo, and removing either breaks that repo instead of reclaiming disk. Which builds this repo needs is a question to ask rather than to remember, since it changes whenever the pin does: `browsers.json` inside the resolved `playwright-core` names them. That also means the flag is not belt and braces here, it is the only thing that keeps two repos installed at once. Nothing in this repository enforces any of it, because what it protects is outside every repository.
...
- For time-dependent logic use `vi.useFakeTimers()` and `vi.useRealTimers()`. Never wait on a real `setTimeout`.
  ...
  - Vitest 3 enforces `testTimeout`, which Vitest 2 did not always do. If a test starts failing on time after a runner upgrade, measure it on the old runner before assuming the new one made it slower: it may simply have started exceeding a limit it always exceeded. Prefer a per-`describe` timeout over raising the project-wide default, and write it as `budget(<ms>)` from `webview/__tests__/helpers/testBudget.ts`: a budget is a base measured with the machine to itself, and the helper scales it by the factor coverage instrumentation adds, which `vitest.config.ts` also applies to the default. `shared/__tests__/testBudgets.test.ts` refuses a fixed number in the three shapes a timeout is written (an option, a positional last argument, that argument on its own line), and a number that is not a budget says so with a same-line `budget-ok:` comment. That rule earned its keep on the move to Vitest 4: the heavy corpus suites timed out under `pnpm test:coverage` on the new runner, and the same command on the old one, same machine, timed out harder. The cause was machine contention, and blaming the upgrade would have reverted the fix for the flake it was landed to remove.
