 ...
 * Writer for Mac actually parses.
 *
 ...
 * the Mac app declines most capabilities and the commands that post those messages are
 * never offered. The defect this guards is that nothing distinguished those
 * from a message that IS reachable in the Mac app and is being dropped, and the two
...
const bridgePath = join(repoRoot, "mac", "Sources", "BirtaWriterCore", "Bridge.swift");
...
/**
 * Page-to-host types the Mac app deliberately does not parse, each with the reason.
 *
...
const DELIBERATELY_UNPARSED: Record<string, string> = {
    ...
    exportHtml: "no export destination: the Mac app has no Save As for a rendered file",
    whatsNewSeen: "the extension's release notes; the Mac app's update offer is its own",

    // ASKED IN THE MAC APP, and deliberately not answered, because the page's own
    // documented behaviour without an answer is the behaviour the Mac app wants. This
    ...
    //
    // The line the Mac app draws, now that four of these have crossed it: it remembers
    ...
    setBlockHandles: "editor state the Mac app does not persist; the default stands each launch",
    setCalcAutoInsert: "editor state the Mac app does not persist",
    setChecklistSink: "editor state the Mac app does not persist",
...
/**
 * Reachable in the Mac app AND dropped: real defects, each with its ticket.
 *
 * This is `it.fails`'s discipline in list form. It must SHRINK. A type here is
 * a button a person can press in the Mac app that does nothing, so moving one out is
...
describe("the Mac app's parse table against the page's outbound vocabulary", () => {
     ...
     * the Mac app had `/ai`, while the page posted `agentCancel`, so clicking the
     * gutter marker to cancel a run did nothing and `AgentRunner.stop` was
     * unreachable from the page.
     */
    it("every case in the Mac app's parse table should be a type the page can actually post", () => {
     ...
     * which no reader can verify and no rename would have updated; and one the Mac app
     * declares, which would make the reason argue the opposite of the truth.
     */
    it("a reason citing a capability should name a real one the Mac app declines", () => {
        ...
        for (const [type, cap] of cited) {
            ...
            expect(HOST_PROFILES.mac, `${type} cites a capability the Mac app declares`)
    ...
    it("no type should be both parsed and listed as declined", () => {
        ...
        expect(both, "The Mac app parses these, so their entry is stale and should be deleted").toEqual([]);
    });
});
