/**
 * The completeness check in `parseJotMenu`, driven rather than read.
 *
 * `parseJotMenu` is the single source two guards stand on, and the only thing
...
import { parseJotMenu, menuSections, JOT_MENU_PATH } from "./jotMenuTable";

const REPO_ROOT = resolve(__dirname, "..", "..");
const SOURCE = readFileSync(join(REPO_ROOT, JOT_MENU_PATH), "utf8");

/** A repo root holding one Swift file: `source`, at the real path. */
function rootHolding(source: string): string {
    const dir = mkdtempSync(join(tmpdir(), "jot-menu-table-"));
    const dest = join(dir, JOT_MENU_PATH);
    mkdirSync(dirname(dest), { recursive: true });
    writeFileSync(dest, source);
    return dir;
}

/** `parseJotMenu` over a mutated copy: the row count, or the message it threw. */
function parseMutated(replace: [string, string]): { rows?: number; threw?: string } {
    ...
    try {
        return { rows: parseJotMenu(rootHolding(mutated)).length };
    } catch (error) {
...
describe("parseJotMenu's completeness check", () => {
    const BASELINE = parseJotMenu(REPO_ROOT).length;

    it("the control should read the whole table, so a throw below is the mutation's", () => {
        expect(BASELINE).toBeGreaterThan(40);
        expect(parseJotMenu(rootHolding(SOURCE)).length).toBe(BASELINE);
    });
