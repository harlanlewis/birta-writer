/**
 * Guard for the Mac app's macOS floor: the three places that state it agree.
 *
 ...
 * documents to the same number. The Mac app's version of that pair is
 ...
 * Mac we say we support. `mac/README.md` is the third, and it is the only one
 * a person reads.
 *
 ...
 * `.github/workflows/mac-app.yml` asks the binary itself.
 */
...
const manifest = read("mac/Package.swift");
const plist = read("mac/Resources/Info.plist");
const readme = read("mac/README.md");
const updater = read("mac/scripts/update.sh");
const workflow = read(".github/workflows/mac-app.yml");
const release = read(".github/workflows/release.yml");
const inApp = read("mac/Sources/BirtaWriter/Updater.swift");
const requirements = read("mac/Sources/BirtaWriterCore/SystemRequirements.swift");
...
describe("the Mac app's macOS floor", () => {
    it("Package.swift should declare a platform floor", () => {
        expect(
            manifestFloor(),
            "mac/Package.swift no longer declares platforms: [.macOS(...)]",
        ).toBeDefined();
    });
    ...
    it("Info.plist should declare LSMinimumSystemVersion", () => {
        ...
        expect(
            plistFloor(),
            "mac/Resources/Info.plist no longer declares LSMinimumSystemVersion",
        ).toBeDefined();
    });
    ...
    it("the README should state a minimum macOS version", () => {
        ...
        expect(stated.length, "mac/README.md states no macOS minimum").toBeGreaterThan(0);
    });
    ...
    for (const [index, version] of stated.entries()) {
        it(`the README's stated minimum #${index + 1} should match the declared floor`, () => {
            ...
            expect(
                ...
                `mac/README.md advertises macOS ${version}, the build requires ${compiled}`,
            ).toBe(true);
        });
    }
});
...
describe("what the update paths refuse", () => {
    it("the by-hand updater should check both axes before it replaces anything", () => {
        ...
        expect(floor, "update.sh no longer reads the bundle's floor").toBeGreaterThan(-1);
        expect(architecture, "update.sh no longer reads the bundle's architectures")
            .toBeGreaterThan(-1);
        expect(quit, "update.sh no longer quits the running app").toBeGreaterThan(-1);
    ...
    it("the by-hand updater should read the floor off the bundle rather than spell one", () => {
        ...
        expect(
            code(updater, "#"),
            "update.sh spells the macOS floor instead of reading it off the bundle",
        ).not.toMatch(new RegExp(`macOS ${floor}\\b`));
    });
    ...
    it("the job that asserts the architecture should build on the runner that publishes it", () => {
        ...
        // release, and the assertion above runs in `mac-app.yml`, which builds a
        ...
        // `mac-app` to an Intel runner and the published download changes
        // architecture while the guard on the claim keeps passing.
        const asserted = jobRunner(workflow, "mac-test");
        const published = jobRunner(release, "mac-app");
        expect(asserted, "mac-app.yml no longer declares a mac-test job with a runner").toBeDefined();
        expect(published, "release.yml no longer declares a mac-app job with a runner").toBeDefined();
        expect(
            published,
            `release.yml builds the published app on ${published} and mac-app.yml asserts its architecture on ${asserted}`,
        ).toBe(asserted);
    });
});
