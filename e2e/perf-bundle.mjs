...
async function main() {
    ...
    if (argv.includes("--set-budget") || argv.includes("--write-baseline")) {
        ...
        if (argv.includes("--set-budget")) {
            ...
            // --write-baseline PRESERVES the ceiling. It no longer refreshes a
            // snapshot, because none is stored; it survives only so the flag in
            // muscle memory and older docs still does something harmless.
