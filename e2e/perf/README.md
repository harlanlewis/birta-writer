...
A sync is four pieces: the serialize, the minimal-diff merge, the live fingerprint, and the verifying reparse of the merged bytes (`utils/verifiedMerge.ts`), and only the last of them is a whole-document parse. Past a size floor all four run in the sync worker and the interaction thread keeps a fifth piece instead, the `doc.toJSON()` snapshot it hands over (MAR-430, MAR-432). The same probe machinery asks the page for all five after the settle marks (`__birtaPerf.syncSplit()`, installed beside the create split in `webview/editor.ts`), and `pnpm perf` prints them as `sync split` and carries them as `syncSplit` in its JSON, with whether this document's syncs take the worker or run on the main thread. Warm, like the create split. It is the reading that says which pieces the interaction thread still pays on a large document, which is what sizes the next tier of the worker pipeline; `pnpm perf huge-outline` is where to read it.
