...
- The post-paint floors move in OPPOSITE directions from launch's 3% / 10 ms: the percent floor is coarser and the ms floor is tighter. Both are deliberate. These spans move more in relative terms than the mount path, and they are small enough that launch's 10 ms would be a large fraction of one rather than a floor under it. `verdict.test.mjs` asserts both directions, so this sentence and the constants cannot drift apart again. Recalibrate with a null A/B, not with `pnpm perf`: a plain measure run gives medians, and what a floor has to clear is the delta between two runs of the same bundle. The command is in the constants' header.
