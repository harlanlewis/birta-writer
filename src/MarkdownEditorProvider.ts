...
import { DiskSyncController } from "./diskSync";
...
export class MarkdownEditorProvider
    ...
    // Reconciles external disk writes into open documents — revert for clean
    // documents, three-way merge for dirty ones, conflict flag when both
    // sides changed the same lines (see src/diskSync.ts). Shares the
    // per-document edit queue and relays conflict transitions to the
    // webview's toolbar badge.
    private readonly _diskSync = new DiskSyncController({
        enqueue: (uriKey, task) => this._enqueueEdit(uriKey, task),
        onConflictChange: (uriKey, conflicted) => {
    ...
    async resolveCustomTextEditor(
        ...
        // Track the file on disk for external writes (terminal tools, git,
        // background sync). VS Code auto-reloads a CLEAN document on its own;
        // the controller covers the dirty case (three-way merge) and acts as
        // a backstop when the auto-reload lags. Disposed with the panel.
        const diskTracking = this._diskSync.track(document, uriKey);
        webviewPanel.onDidDispose(() => diskTracking.dispose());
            ...
            async (message: ToExtensionMessage) => {
                ...
                switch (message.type) {
                    case "ready": {
                        ...
                        if (this._diskSync.isConflicted(uriKey)) {
                            webviewPanel.webview.postMessage({
                                type: "syncConflict",
                                state: "conflict",
                            } satisfies ToWebviewMessage);
                    ...
                    case "resolveSyncConflict":
                        void this._diskSync.resolveConflictInteractively(document, uriKey);
                        break;
                }
            },
        );
        ...
        const changeSubscription = vscode.workspace.onDidChangeTextDocument((e) => {
            ...
            // A clean document after a content change was just synced with
            // the disk (auto-reload, revert, undo back to the save point) —
            // the disk-sync controller re-anchors its merge base there.
            this._diskSync.noteDocumentChanged(e.document, uriKey);
