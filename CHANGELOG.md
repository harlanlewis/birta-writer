...
- Ask your own AI agent for an edit without leaving the document. Type `/ai`, press Space, write what you want in the same line, and press Enter: Birta saves the file, then hands your agent one prompt naming that file and the lines your cursor sits in. It runs the agent you already use, in a terminal you can watch, rather than sending anything anywhere itself. Off by default (`birta.ai.enabled`), because picking it runs a program on your machine. `birta.ai.command` is the command line it runs, defaulting to the Claude Code CLI (`claude ${instruction}`) and pointable at any agent: the placeholders `${instruction}`, `${prompt}`, `${reference}` and `${file}` are filled in quoted, so a prompt containing quotes or `$` reaches the agent as you typed it. The result arrives the way any other change to the file on disk does, and if you have edited the document since asking, it arrives as the usual notice that the file changed rather than as an edit.
...
- Link cards: a web link that sits alone on its own line, bare or `[labelled](url)`, can render as a quiet card of the page's title, description and site, read from the page's own Open Graph metadata; the readable URL stands in until the page answers. Off by default (`birta.linkCards.enabled`, with a Toggle Link Cards command), and it needs `birta.network.enabled` because the card fetches the page. Or leave the default off and choose per link: the block menu on a lone link offers Show as Card and Show as Link, as do the card's own control and its edit palette. Nothing is written to your file, no image is fetched, only the link's own site is contacted (and, if it redirects, the site it sends you to, each hop checked the same way), once per session, and a link a provider card already handles keeps its provider card; a provider link whose provider you switched off stays a plain link unless you choose a card for it.
