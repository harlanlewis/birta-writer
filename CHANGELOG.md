...
- A file explorer, on a host that opens a folder (Birta Writer for Mac's folder windows; VS Code has its own and is unchanged). It is a sidebar to the left of the document, drawn and driven like the table of contents: the same docked and floating modes, the same drag edge for its width, the same hover flyout when it is collapsed, and the same arrow-key navigation, with Right and Left opening and closing folders. It lists the folder as a tree, folders first, one level at a time as you open them, with files the editor does not open dimmed and still clickable, and hidden dotfiles left out until the host's Show Hidden Files is on. The row for the open file is selected and scrolled into view whenever the file changes, whichever way it was opened. A folder button in the toolbar shows and hides it and is where the flyout opens from. When the table of contents is on the left too, the two sit side by side, files outermost. A window on a single file has none of this, and in Birta Writer for VS Code the panel, its stylesheet and the code behind it are never fetched.
