...
- A save now checks that the bytes it is about to write reopen as the document on your screen, and writes the editor's own canonical formatting for that one save when they would not. This closes the ways a move inside a widely indented outline could still damage the file after the fix below, which closed one of them. The editor writes two spaces per list level and plenty of files use four, so saving one restores your spelling over the editor's; where a line moved into an item kept the editor's narrower indent while the item's own marker line was restored to your wider one, the item's content ended up further left than its marker allows, and the lines under it were far enough past that point to reopen as a code block or to run together into a single paragraph. It needed an outline only three levels deep, which is what a Notion or Obsidian export of an ordinary nested list looks like, and nothing on screen showed it: the file looked right until it was closed and opened again. The fallback reformats more of that one file than a save usually does, which you can see in a diff and undo. Losing the structure could not be undone, which is the trade.
