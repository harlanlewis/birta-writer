...
- Ask your agent from the caret. Type `/ai`, Space, then a request in plain words (`/ai add a mermaid diagram of the flow above`) and Enter: Birta composes one line, your request plus a `path.md#L12` reference to the caret, saves the document so that reference names what is on disk, and hands the line to the agent you already run, per `birta.agent.command`: a shell command such as `claude -p {prompt} --permission-mode acceptEdits`, `chat` for VS Code's Chat view, or `clipboard`. It asks which once, on first use, and stores the answer in your user settings; the setting is never read from a workspace. A shell command runs in the background by default (`birta.agent.mode`): no terminal opens, a small filled pill with a stop square sits in the gutter beside the line the request was typed on while the agent works (its tooltip names the harness, `claude` or `codex`, and clicking it stops the run), and when the agent finishes its edit lands in the editor and undoes with Cmd+Z in one step, like a paste. Every run ends with something you can see: a changed file plus a status-bar line, or, when the agent changed nothing (say hello), a message carrying its last words; a failure is a message too, and the pill turns into an error mark. If you kept typing while it worked, the edit is merged around yours; a change that overlaps something you deleted is left out and the pill says so, with the agent's version on disk. Set the mode to `terminal` to watch the run in one reused Birta AI terminal instead (a terminal is a hand-off the editor cannot follow, so no pill). Nothing is sent to a model by Birta itself, and it is one request each time, not a conversation. Ask Agent in the command palette does the same and asks for the request in an input box, as does `/ai` then Enter with nothing typed. Typed on a fresh empty line below other content, the request stays on that line (its marker sits beside it) and the reference names the blank line after the block above, since an empty line has no line of its own in the file. In the slash menu, Space commits only the Ask Agent row and only once you have typed its name (`/ai`, or a full keyword such as `/agent`); on every other row, and on a shorter query, Space stays a filter character, so `delete table` still filters.
