...
- **URL embeds** — a YouTube link on its own line renders as an inline player card: a static thumbnail with a play button that loads the actual player only when you click it (through YouTube's privacy-mode `youtube-nocookie.com`, and only then). The card is render-only — the source stays the plain link, so the file round-trips byte-for-byte — and clicking into the line reveals the raw URL to edit. The thumbnail loads on an idle pass after the editor is interactive, never blocking first paint. Gated by `birta.embeds.enabled`.
