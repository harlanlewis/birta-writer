...
- **Proofreading starts when you start working**: spelling, grammar, and style underlines now appear on your first interaction with a document (a click, keystroke, or paste) rather than the instant it opens. Opening a file just to read it no longer spins up the grammar engine, which makes opens noticeably faster and lighter; once you engage, checks behave exactly as before. Toggling any check on from the palette or toolbar also runs it immediately.
