...
- **Inline calculator** — type an arithmetic expression right before an equals sign (`12 * 4 =`, `(3 + 4) / 2 =`) and the result is offered as a suggestion you confirm with Tab (Return stays free to start a new line); it inserts as plain text, so the file stays ordinary prose. Supports `+ - * /`, `%` (modulo), `^` / `**` (exponent), parentheses, decimals, and unary minus; only pure arithmetic is evaluated (never anything with letters), and division by zero or malformed input simply offers nothing. Detection is kept narrow: it stays out of code blocks, and prose that merely *contains* arithmetic-looking characters — dates (`2026-07-17 =`), phone-style numbers, algebra with a variable (`x - 4 =`), or a comma-grouped number's tail (`1,000 + 2 =`) — is refused rather than mis-evaluated. On by default (`birta.calc.enabled`); set `birta.calc.autoInsert` to have the result inserted the moment you type `=` instead of confirming it.
