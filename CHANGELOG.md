...
- Moving a nested list in a document that indents with four spaces per level no longer writes it back at the wrong depth. In a file whose outline is spelled more widely than this editor's own two spaces, dragging a sub-list could save those lines several levels deeper than any level the document actually had, deep enough that they stopped being list items at all and reopened as run-together text with the bullets showing as literal characters. The editor learns a file's indent convention from opening and re-saving it unchanged, and for a moved block it was applying that convention twice over: once restoring each line's saved spelling, then again placing the block at its new depth. It now applies it once, and where a moved line cannot be placed in the file's own spelling honestly it writes the editor's own indent rather than guessing at one. This was not limited to the deeply nested case that made it obvious: a substantial share of the drags available in such a file could lose content, and the editor's own check for this damage could not see it, because the check compared the result against bytes that already carried the fault. One case is still open, and it is worth knowing if you keep outlines this way: in a file where four spaces begins both a bulleted and a numbered list, moving a block can still leave one line of an outline you never touched written at the editor's indent while the line under it keeps the file's, which reopens the deeper line as run-together text.
