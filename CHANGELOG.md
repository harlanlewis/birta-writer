...
- Where such a move cannot be made safe, the editor now blocks it instead of writing a broken file. Dragging a paragraph out of a quote nested inside a list item, in one of these outlines, previously appeared to work while leaving behind a marker that split the list apart on reopen. It now leaves the document untouched and says "Move blocked, the result would not survive saving and reopening." The check that decides this did not recognize a list item whose content starts with a quote, heading, or code block and holds more than one block, which is why it had stayed silent. It guards block moves and drags; typing and pasting are governed separately.
