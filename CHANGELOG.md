...
- GitHub links can show live detail. With the network switch and URL Embeds on, connecting your GitHub account through the new Connect Service command lets a repository, issue or pull request link render with its real title and state instead of only what the address spells out. A link you have not connected shows the same card it always did, with a quiet one-click offer to connect that dismisses for the session if you would rather it did not ask again. The credential is held by VS Code's own GitHub sign-in and never reaches the document or the editor's webview; the request goes only to GitHub's own API host, carries only the id in your link, and nothing fetched is ever written into your file. Disconnect Service removes it.
