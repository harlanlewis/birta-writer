...
- An invalid Mermaid diagram froze the window on document open. A diagram Mermaid cannot parse (one mistyped bracket is enough) sent the renderer into an endless retry the moment the preview first drew it, on the same thread everything else runs on: the window stopped responding before the first keystroke and macOS offered to kill it. Valid diagrams elsewhere in the same document made no difference; one bad fence was sufficient. A failed render now settles on an error card showing Mermaid's own parse message, and renders again only when the diagram's text or the effective theme actually changes. The fullscreen view also stops opening on the error card as though it were a diagram.
