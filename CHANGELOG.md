...
- Birta Writer is now published to Open VSX as well as the VS Code Marketplace, so it installs from the built-in extension search in VSCodium, Cursor, Windsurf, Gitpod and the other editors that read that registry, rather than needing a `.vsix` downloaded and installed by hand. It is the same file in both registries and on the GitHub release, covered by the same build-provenance signature, so an install from any of them can be checked against this repository and this commit.
