...
- Ordered lists can be drawn with letters or roman numerals. A new Numbering section in a list's gutter menu offers `1.`, `a.`, `A.`, `i.` and `I.`, and typing `a. `, `A. `, `i. ` or `I. ` where you would type `1. ` starts a list in that style. Typing one of those markers on a line directly below an existing numbered list, or at the start of an item in one, restyles that list instead of starting a second one. Nested levels already alternated decimal, letters, roman by depth; this is the same choice made explicit at any level. Your file always gets ordinary `1.` markers, because CommonMark has no lettered or roman marker and `a. alpha` is a paragraph to GitHub and to VS Code's own preview: the list stays a normal numbered list that Birta draws differently, so it stays portable. The choice is remembered per list alongside your folds and widths, which means it survives closing the file but lives with the workspace rather than in the document. One consequence worth knowing: `a. alpha` typed at the start of a line becomes a list, so a paragraph genuinely opening with an initial (`A. Smith said`) converts. Backspace immediately after puts the characters back as text, and Cmd+Z forgets them.
...
- Setting one table to Full Width changed a different table too. Any two tables sharing a header row (`| Name | Age |`, the common case) were treated as one block by the width control, and Duplicate makes such a pair in a single click, so widening a duplicated table widened its original and the reverse. Editing one of their header rows was worse. It moved the single stored preference to whichever table you typed in, so the other kept its width on screen but reverted the next time you opened the file. The same coupling applied to two code blocks opening on the same first line, the same image used twice, and the same URL embedded twice. Each block now keeps its own width, and Duplicate hands the copy the width of the block it copied. Nothing about this was ever written to your file. Two limits are worth knowing, both unchanged in kind from before: these preferences live with the workspace rather than in the document, and because identical blocks are told apart by their order in the file, reordering two of them swaps their widths.
