...
- **Floating selection palette** — selecting text raises a small formatting bar just above it (paragraph/heading, bold, italic, strikethrough, inline code, highlight, link, clear formatting, inline math), for mouse and keyboard selections alike; selecting whole blocks raises move / duplicate / delete instead — the first mouse affordance for a multi-block selection. Its Link button drives the same link editor as the toolbar and ⌘K. Turn it off with `birta.floatingToolbar.enabled`, or hide individual buttons with `birta.floatingToolbar.items.*`.
...
- **Links and wikilinks** — one link editor for both inserting and editing (⌘K, or click a link; applies on blur; a Markdown / `[[wiki]]` format switch). Wikilinks (`[[target|alias#heading]]`) round-trip byte-identically with bare-name autocomplete. **Smart link resolution** (`birta.smartLinks`) opens local links the way a site generator would publish them — workspace-root paths, content-root inference, `index.md` / `_index.md` suffixes — and shows the resolved target while you edit.
...
- **Resizable Table of Contents** — a chrome-free TOC docks on either side (`birta.tocPosition`), resizes by dragging, hides to a small tab, and switches sides in place. Drag TOC items to reorder whole sections, or drag blocks into a section to refile them.
