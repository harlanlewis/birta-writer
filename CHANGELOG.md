...
- **Paste a bare URL to get a titled link** — with nothing selected, pasting a bare URL (`https://…`, or a bare domain like `example.com`) inserts a link and, when network features are enabled, fetches the linked page's title and upgrades it to `[title](url)`. The link appears instantly and the title fills in a moment later; it's one undo step. The fetch requests only the URL you pasted, reads the page's title from its HTML (no third-party service), and falls back to the plain link when the page is offline or untitled. Offline by default: with the master network switch `birta.network.enabled` off — the default — the paste inserts a plain link and makes no request, and a quiet, dismissable "Fetch link title? · Enable" prompt appears at the link; clicking Enable turns network features on and fetches that link's title on the spot. The feature also has its own `birta.pasteUnfurl.enabled` (on by default). Pasting a URL *over selected text* still links the selection as before, and detection stays narrow (a file path or `notes.md`-style name is never treated as a URL).
