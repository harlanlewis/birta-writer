...
- Compare a Markdown file with its last committed version as a rendering rather than as source. `Birta Writer: Compare with HEAD` opens a panel beside the document showing it the way the editor draws it: a word edited mid-sentence is marked as that word rather than the whole line, a removed paragraph is shown where it was, and `j`/`k` or the header arrows step between changes against a count. Reach it from the command palette, the editor title bar, or by right-clicking a changed file in Source Control. It follows the open buffer rather than the file on disk, so unsaved edits are part of what you review, and it re-reads when you come back to the panel after committing. VS Code's own diff is unchanged and is still the right tool when you want the source. Images show as their path and inline HTML as its source rather than rendering.

- Typewriter mode holds the line you are editing at a fixed height on screen and scrolls the document under it, instead of letting the active line drift toward the bottom of the pane. `birta.typewriterMode`, off by default, with `Birta Writer: Toggle Typewriter Mode` to flip it. It applies to typing and keyboard navigation only: clicking somewhere else leaves the view where it is, and a selection you are extending is never scrolled out from under you. Nothing about the file or how it renders changes.
...
- Birta Writer Jot's Settings window is titled for the app rather than for the pane you are on, so a window from an app with no Dock icon says which app it belongs to. Its rows use the smaller switch and its groups no longer draw an outline, which is the shape macOS System Settings uses.
