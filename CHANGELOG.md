...
- Moving a nested list in a document that indents with four spaces per level no longer writes it back at the wrong depth. In a file whose outline is spelled more widely than this editor's own two spaces, dragging a sub-list could save those lines several levels deeper than any level the document actually had, deep enough that they stopped being list items at all and reopened as run-together text with the bullets showing as literal characters. The editor learns a file's indent convention from opening and re-saving it unchanged, and for a moved block it was applying that convention twice over: once restoring each line's saved spelling, then again placing the block at its new depth. It now applies it once, and where a moved line cannot be placed in the file's own spelling honestly it falls back to the plain two-space form rather than guessing. This was not limited to the deeply nested case that made it obvious; most drags in such a file could lose content, and the editor's own check for this damage could not see it, because the check compared the result against bytes that already carried the fault.

- The bullet character you type is now honored wherever you type it. Typing `* ` on the line below a `-` list used to continue that list and silently drop the character, even though those same two lines read from a file are two separate lists; it now starts a second list spelled `*`. The ordered delimiter follows the same rule, so `2) ` below a `1.` list keeps its `)`. Typing the character the list above already uses still continues that list, as it did before. One related case comes with it: deleting the paragraph between a `-` list and a `*` list used to merge them and drop the `*`, and now keeps both lists. Merging two differently spelled lists is still offered, in the block menu and as a quiet prompt at the cursor, so what changed is that it is no longer done for you unasked.
