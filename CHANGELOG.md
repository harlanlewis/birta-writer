...
- **URL embeds** — a YouTube link on its own line renders as an inline player card: a static thumbnail with a play button that loads the actual player only when you click it (through YouTube's privacy-mode `youtube-nocookie.com`, and only then). The card is render-only — the source stays the plain link, so the file round-trips byte-for-byte — and clicking into the line reveals the raw URL to edit. A quiet corner button opens the video on YouTube in your browser — the in-editor player depends on YouTube accepting the embed context, the external path always works. The thumbnail loads on an idle pass after the editor is interactive, never blocking first paint. Offline by default: the card renders and the thumbnail loads only when the master network switch `birta.network.enabled` is on (and the feature's own `birta.embeds.enabled`, on by default); with the master off — the default — no card is shown and nothing is requested from YouTube.
...
- **Paste a bare URL to get a titled link** — with nothing selected, pasting a bare URL (`https://…`, or a bare domain like `example.com`) inserts a link and, when network features are enabled, fetches the linked page's title and upgrades it to `[title](url)`. The link appears instantly and the title fills in a moment later; it's one undo step. The fetch requests only the URL you pasted, reads the page's title from its HTML (no third-party service), and falls back to the plain link when the page is offline or untitled; it refuses local and private-network addresses outright (localhost, `192.168.*`-style ranges, cloud metadata endpoints) and re-checks every redirect against the same rule, so a pasted link can only ever reach the public web. Offline by default: with the master network switch `birta.network.enabled` off — the default — the paste inserts a plain link and makes no request, and a quiet, dismissable "Fetch link title? · Enable" prompt appears at the link; clicking Enable turns network features on and fetches that link's title on the spot. The feature also has its own `birta.pasteUnfurl.enabled` (on by default). Pasting a URL *over selected text* still links the selection as before, and detection stays narrow (a file path or `notes.md`-style name is never treated as a URL).
