...
- **Unlink button in the link popup header**: the remove-link control is now a broken-chain **unlink** icon in the popup's header row (next to open and edit), replacing the trash can that lived at the bottom of the edit panel. The icon reads as "remove the link, keep the text" (which is what it does), and it's one click from hover without opening the editor. It's hidden for read-only links (reference / same-page wikilinks) and while inserting a brand-new link.
