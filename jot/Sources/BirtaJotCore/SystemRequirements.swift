...
/// Whether a Mac can run a given build of Jot, decided from the build itself.
///
...
///
/// It exists for the update paths. `Updater` and `jot/scripts/update-jot.sh`
/// both quit the running app and replace it, and their swap scripts are built
/// around never leaving somebody with no Jot at all. An update the machine
...
public enum SystemRequirements {
    ...
    /// binary Jot was not built to produce, and reading it as "some other
