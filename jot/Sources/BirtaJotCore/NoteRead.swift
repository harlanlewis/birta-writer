...
/// is a data-loss bug rather than a tidiness one: Jot mounts the empty buffer,
...
/// where nothing but Jot ever touched it, so nil really did mean "no note
...
public enum NoteRead: Equatable, Sendable {
    ...
    /// into it — and Jot is refusing to write, so that typing would go
