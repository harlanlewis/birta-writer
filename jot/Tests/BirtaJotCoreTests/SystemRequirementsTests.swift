...
/// Whether a Mac can run a build of Jot, decided before that build replaces
...
final class SystemRequirementsTests: XCTestCase {
    ...
    func testOnlyACompatibleVerdictShouldHaveNothingToSay() {
        ...
        XCTAssertNil(SystemRequirements.refusal(.compatible, productName: "Jot"))
        for verdict in refusals {
            let message = SystemRequirements.refusal(verdict, productName: "Jot")
            XCTAssertNotNil(message, "\(verdict) should say something")
            XCTAssertTrue(message!.contains("Jot"), "\(verdict) should name the product")
            XCTAssertTrue(
    ...
    func testTheRefusalShouldNameBothTheBuildAndTheMachine() {
        let tooOld = SystemRequirements.refusal(
            .systemTooOld(needs: "15.0", running: "14.7"), productName: "Jot")
        XCTAssertTrue(tooOld!.contains("15.0"))
        XCTAssertTrue(tooOld!.contains("14.7"))
        let wrongArch = SystemRequirements.refusal(
            .wrongArchitecture(built: [.arm64, .x86_64], machine: "ppc"), productName: "Jot")
