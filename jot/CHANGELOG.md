...
- Birta Writer Jot's agent command has a menu of the terminal agents people actually run: Claude Code, Codex CLI, Cursor CLI, Gemini CLI, GitHub Copilot CLI, OpenCode, Aider, Amp and Goose. It reads Select Template whether or not you have used it, because choosing one fills the command field below and is then done with: that field is where the setting lives, and it is yours to edit. Settings, Editor.
...
- What a summon opens is now called New windows open with, in Settings, Editor, and its two answers are Last open file and New file. It is the same setting, named for what you get rather than for the policy behind it; it was Opens in Advanced, and before that Start with a blank note.

- `/ai` has a switch of its own, Enable /ai commands, in Settings, Editor. It runs a command on your Mac, and a capability that runs commands is one to turn on deliberately rather than one to find already running, so a new install starts with it off. If you already have Jot, nothing changes: `/ai` shipped working, and a new default is not a reason to take it away, so it stays on and the switch simply shows you where it lives. With it off, `/ai` is not offered anywhere, in the slash menu or the command palette, and the command it would run is remembered either way, so turning it back on does not ask you the question again.

- Birta Writer Jot's Settings is three panes, split by what the rows are about. General is Jot as an application on this Mac: how you summon it, where it puts your notes, whether it writes as you type, appears in the Dock, starts at login and keeps itself current. Editor is what happens inside the panel: which note a summon opens, what a new one is called, and the agent `/ai` hands a prompt to. Advanced holds the two gestures that undo rather than set. Every question the first-run screen asks is in General, in the same order and worded the same, so a setting you answered on first run is found again by looking where you answered it. The headings over the groups are gone, since a card of switches is bounded by its own shape and a title over it named what the rows already said, and the window no longer slides between sizes when you change panes.
