...
- `/ai` has a switch of its own, Enable /ai commands, in Settings, Editor, and it starts off. It runs a command on your Mac, and a capability that runs commands is one to turn on deliberately rather than one to find already running. This applies to an install you already have, not only to a new one: if you were using `/ai` before this version, it is off now and the switch is where you turn it back on. The command it would run is remembered either way, so turning it on does not ask you that question again. With it off, `/ai` is not offered anywhere, in the slash menu or the command palette.
