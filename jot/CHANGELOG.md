...
- `/ai` is off until you switch it on. Settings, Editor, Enable /ai commands, which ships off and takes the agent rows with it. It runs a command on your Mac, and a capability that runs commands is one to turn on deliberately rather than one to find already running; the command itself is remembered either way, so switching it on does not ask you the question again. With it off, `/ai` is not offered anywhere: not in the slash menu, not in the command palette.
