...
`AppIcon.icns` is the app icon, named by `CFBundleIconFile` in `Info.plist`. macOS does not round an app icon for you, so `make-icons.sh` cuts the artwork to the shape: Apple's grid puts 824 square of art on a 1024 canvas at corner radius 185.4, with the margin the system's own drop shadow is drawn into. The mark is a single word drawn at the canvas's own scale, so it still reads in the 32 square slot; the 16 square one is below what any lettering survives, and there it is a coloured tile. That is a property of the mark rather than of the export.

`MenuBarTemplate.pdf` is the menu-bar mark. PDF because a status item is drawn at the display's own backing scale, and a template image because macOS renders one from its alpha alone, which is what inverts it for a dark menu bar and for the highlighted state. It is drawn deliberately smaller than the bar's own thickness, which `statusItemImage` in `App.swift` sets: the SF Symbols beside it carry their padding inside the box they are asked for, so matching the size they are given draws a visibly larger neighbour. The number is about the drawn height of the glyph rather than the shape of the artwork, which matters because the artwork has changed under it: a mark that is a bare letterform and one that is a filled box land at the same height only because the letterform fills most of its own box. `swift run` has no bundle to read it out of and falls back to a stock symbol.
